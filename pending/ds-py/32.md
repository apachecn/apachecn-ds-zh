...
请参阅 [http://lasagne.readthedocs.org/en/latest/user/installation.html](http://lasagne.readthedocs.org/en/latest/user/installation.html) 了解安装说明。
...
神经网络是层的集合。 在`nolearn`中实现一个层是组织这些层的外观的情况，就像在 PyBrain 中一样。 我们在[第 8 章](29.html "Chapter 8. Beating CAPTCHAs with Neural Networks")和*中使用神经网络*击败 CAPTCHAs 的神经网络使用了完全连接的密集层。 这些是在 `nolearn`中实现的，这意味着我们可以在此处复制我们的基本网络结构。 首先，我们创建由输入层，密集隐藏层和密集输出层组成的层：
...
您可以在 [http://markus.com/install-theano-on-aws 上获取的详细信息，以及可能会提供有关在另一台计算机上进行设置的信息。 /](http://markus.com/install-theano-on-aws/) 。
...
为简便起见，我们已经安装了`Lasagne`，并将以系统级软件包的形式安装`nolearn`。 对于那些想要更便携的解决方案的人，我建议使用`virtualenv`安装这些软件包。 它将允许您在同一台计算机上使用不同的 python 和库版本，并使将代码移动到新计算机变得更加容易。 有关更多信息，请参见 [http://docs.python-guide.org/en/latest/dev/virtualenvs/](http://docs.python-guide.org/en/latest/dev/virtualenvs/) 。
...
要更改批处理大小，您将需要创建`BatchIterator`实例。 对此参数感兴趣的人可以在 [https://github.com/dnouri/nolearn/tree/master/nolearn/lasagne](https://github.com/dnouri/nolearn/tree/master/nolearn/lasagne) 上查看文件的源，跟踪`batch_iterator_train`和`batch_iterator_test` 参数，以及如何在此文件的`NeuralNet`类中设置它们。
