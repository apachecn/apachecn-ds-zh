...
R 也受 CRAN（综合 R 存档网络， [http://cran.r-project.org/](http://cran.r-project.org/)）支持。 CRAN 包含一个可访问的 R 早期版本的存档，可根据要复制的软件的较早版本进行分析。 此外，CRAN 包含数百个免费下载的软件包，从而极大地扩展了 R 的功能。实际上，R 已成为包括统计在内的多个学术领域的默认开发平台，从而导致在 R 中首先实现了最新，最好的统计算法。 在[这个页面](https://mran.microsoft.com/)的 Microsoft 变体中提供了更快的 R 版本。
...
您可以从[这个页面](http://www.PacktPub.com/support)下载从您的帐户[购买的所有 Packt 书籍的示例代码文件。 如果您在其他地方购买了此书，则可以访问](http://www.PacktPub.com/support) [http://www.packtpub.com/support](http://www.packtpub.com/support) 并注册以将文件直接通过电子邮件发送给您。
...
*   适用于 Windows 的 Python 软件包的各种安装程序，位于[这个页面](http://www.lfd.uci.edu/~gohlke/pythonlibs%20)
...
*   有关熊猫的更多信息，请参考[这个页面](http://pandas.pydata.org/)上的 *Python 数据分析库*文章。
...
*   访问位于[这个页面](https://developer.apple.com/xcode/downloads/%20)的 XCode 下载页面。
