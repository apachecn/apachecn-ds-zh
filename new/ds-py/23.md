...
如果您在安装`matplotlib`时遇到任何困难，请在 [http://matplotlib.org/users/installing.html](http://matplotlib.org/users/installing.html) 中查找官方安装说明。
...
（图片来源： [https://www.flickr.com/photos/geckzilla/16149273389/](https://www.flickr.com/photos/geckzilla/16149273389/) ）

可以从 UCL 机器学习数据存储库下载，其中包含用于不同数据挖掘应用程序的大量数据集。 转到 [http://archive.ics.uci.edu/ml/datasets/电离层](http://archive.ics.uci.edu/ml/datasets/Ionosphere)并单击**数据文件夹**。 将，`ionosphere.data`和`ionosphere.names`文件下载到计算机上的文件夹中。 对于此示例，我假设您已将数据集放在主文件夹中名为 Data 的目录中。
