...
如果我们不能确定，则将其称为未解决的问题。 但是，这种区别不仅仅针对作者归属-在实际训练中可能没有实际班级的任何数据挖掘应用程序都被认为是一个开放性问题，任务是查找候选作者或不选择任何作者。
...
我们将在本章中使用的数据是 Gutenberg 项目在 [www.gutenberg.org](http://www.gutenberg.org) 上的一组书籍，该书籍是公共领域文献作品的存储库。 我用于这些实验的书籍来自不同的作者：
...
这些方程式的推导超出了本模块的范围，但我建议感兴趣的读者详细阅读 [http://en.wikibooks.org/wiki/Support_Vector_Machines](http://en.wikibooks.org/wiki/Support_Vector_Mach) 的推导。 另外，您可以访问 [http://docs.opencv.org/doc/tutorials/ml/introduction_to_svm/introduction_to_svm.html](http://docs.opencv.org/doc/tutorials/ml/introduction_to_svm/introduction_to_svm.html) 。
...
可在 [https://www.cs.cmu.edu/~./enron/](https://www.cs.cmu.edu/~./enron/) 上获取完整的 Enron 电子邮件的集。

### 注意

完整的数据集为 423 MB，压缩格式为`gzip`。 如果您没有基于 Linux 的计算机来解压缩（解压缩）该文件，请获取其他程序，例如如 7-zip（ [http://www.7-zip.org/](http://www.7-zip.org/) ）。
