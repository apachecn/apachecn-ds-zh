...
![](assets/61803f58-7eeb-4b09-9f07-b4ad5fd18c40.png)
...
![](assets/2e7ae030-b42a-4653-87ac-131d95b825f0.png)

通常，在窗口上执行简单的移动平均； 因此，使用目标函数在最佳窗口上评估估计的预测，以最大程度地减少误差：

![](assets/a36232e5-d9ab-41a9-9a91-b631a0870b4d.png)

![](assets/1fd9e964-00a3-4016-abd3-56f433192a93.png)
...
![](assets/f6b22f57-500a-4bf2-8a5e-3077aec1ac63.png)
...
![](assets/b01da3e8-3466-4e83-9a3c-f4198ec4c700.png)
...
![](assets/91365cc9-7945-445b-8986-295753e9aefe.png)
...
![](assets/384a8e4e-aed6-4b15-830b-0964122aaff8.png)

图 3.6：样本 IBM 收盘股价数据集

在单平滑指数方法中，预测值生成如下：

![](assets/5c59e282-cdfb-4347-9ba2-8542f47649c3.png)

![](assets/671f62d6-5dbc-4b91-b601-ef1ad6d53165.png)

...

![](assets/7fce3711-4813-420f-843a-a3ca622588c3.png)
...
![](assets/f552f98c-d947-4119-98be-060282aa9c84.png)
...
![](assets/4465e7a5-136e-4958-a0c7-67471f61ffc0.png)

图 3.8：实际值与预测值之间的比较

可以使用标准目标函数，例如**平均平方误差**（ **MSE** 或**平均绝对误差**（ **MAS** ）：

![](assets/6cb1ee4f-f36c-4f2e-9a38-308e2735a5f3.png)

同样，MAS 的评估如下：

![](assets/1d3aff63-032a-4a64-a4a9-5b2fd937e98a.png)
...
![](assets/918516e2-17cb-4958-b984-94282c7d6986.png)
...
![](assets/b3dee9f2-81cf-4728-8541-a19a85a1e2d0.png)

解决前面的方程式将导致以下差异：

![](assets/bad21b72-bc0f-4ab0-a745-1ea65f62b3c8.png)

在此， *T* 是时间序列的长度。 对于系列 *x <sub class="calibre26">T</sub>* 的单位方差，由预测系列捕获的方差将基于平滑参数α改变，如下所示：

![](assets/3fe52f52-4199-4e99-8ad5-9e59e18396fb.png)
...
![](assets/a688817a-d286-47c4-9e6e-133a78169c69.png)

在此， *T <sub class="calibre26">t</sub>* 捕获指数平滑的趋势分量，并表示如下：

![](assets/51921707-dc7c-4e46-a53c-90ef06a9956c.png)

在此，**α**是数据平滑因子，**β**是趋势平滑因子，其值在[0，1]之间。 下一阶段的预测可以如下生成：

![](assets/53ed1c1d-e8a9-4676-883e-9644b29e7897.png)

在二阶平滑中，趋势分量的初始值可以通过多种方式分配：

![](assets/577c5299-17a5-428d-9a4c-4cc95401c9b9.png)

![](assets/d89bbadd-10c5-4699-96b2-f3522af9a898.png)
...
![](assets/a301947c-bbdb-479a-83cc-2d1bc94cffd7.png)

图 3.11：二阶指数平滑中的 alpha 和 beta 平滑参数对啤酒销售数据集的影响

当α= 0 时，初始值保持恒定； 因此，趋势参数不起作用。 但是，当α= 1 和β= 1 时，二阶指数平滑可以写为：

![](assets/b71f8a51-d8a2-41ae-b7de-543ec5a60aaa.png)

![](assets/0ca9995f-a275-4869-aafa-bd6095f48971.png)

![](assets/3d3c5842-910c-4dd2-b0af-361be9190dee.png)

时间 *t* 时的预测取决于先前的值和趋势成分。 当β设置为零时， *t-1* 的趋势分量将取决于 *t-2* ：

![](assets/c220f0ac-d318-42cf-b597-c5aacf2db3ff.png)

因此，趋势分量值取决于初始分配值，并且是一个常数。

类似地，对于α= 1 和β= 1，二阶指数平滑可以简化如下：

![](assets/a98a43c3-54b2-4b16-ba1a-59e731bdac71.png)

![](assets/e8882368-44ee-4875-8c6b-6e5d5e116af4.png)

![](assets/04ee0ba1-8178-4df7-8c2d-0f6e1fb3523a.png)
...
![](assets/e6aaf3ed-6055-470d-a7ac-4af7f9fd5209.png)

图 3.12：实际和双指数预测值之间的比较

我们还比较一下单指数和双指数平滑的性能：

![](assets/e93f4eba-0655-43c8-99e6-6fdddd77d14f.png)
...
![](assets/7332fa27-628c-4f06-b559-30630ba299f1.png)
...
![](assets/7d02438c-99b1-43eb-9c43-0a98501ae069.png)

![](assets/112a1f53-4c4b-49c6-bd28-96ce264f1106.png)

...

![](assets/a9dfa380-ab75-4711-a008-12ee19aa4172.png)
...
![](assets/2c432afb-2576-45de-a33c-cc1304ba26fd.png)
...
![](assets/d5834b17-3a13-4c1d-bc39-dfaada2d6ed3.png)

前面的等式可以推广到任何周期 m，如下所示：

![](assets/5241995d-62ed-490e-9983-6f083e5b1b2b.png)
...
![](assets/6f24ce6d-54a4-473e-b441-06196ffa9c3d.png)

图 3.14：威斯康星州员工数据集

上图显示了威斯康星州员工时间序列数据集。 该数据集包括年度趋势和每月季节性。 我们知道数据的季节性模式。 因此，季节性信息可用于使用以下公式得出趋势的初始值，作为整个季节的平均值：

![](assets/17af71d2-2358-49ed-8370-dccd8e8abdc9.png)
...
![](assets/d9bd37a0-cf89-4187-906f-3a78bddc90ec.png)
...
![](assets/29931b68-c8d9-46d4-b791-bf19ae5fd37a.png)
...
> ![](assets/e94359cf-b390-419f-9ca6-5fd335782f81.png)
...
![](assets/3d35a7ef-c96d-43ac-915c-acd196f73191.png)

图 3.18：实际与三次指数平滑比较

可以使用保留样本上的均方根误差来优化预测。 让我们比较一下单，双和三重指数平滑的性能，如下图所示：

![](assets/40a0400b-8062-4d77-8563-86ff5bb00d77.png)
