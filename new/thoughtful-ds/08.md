...
[**注意**：在统计中，统计假设检验包括通过抽取样本并确定主张是否成立来质疑所提出的假设是否成立。 我们看一下 p 值（ [https://en.wikipedia.org/wiki/P-value](https://en.wikipedia.org/wiki/P-value) ），它有助于确定结果的重要性。 有关统计假设检验的更多详细信息，请参见此处](https://en.wikipedia.org/wiki/Statistical_hypothesis_testing)。
...
[[https://github.com/DTAIEB/Thoughtful-Data-Science/blob/master/chapter%208/sampleCode23.py](https://github.com/DTAIEB/Thoughtful-Data-Science/blob/master/chapter%208/sampleCode23.py) 我们使用了`pprint`软件包，该软件包对于*漂亮- 打印*任何 Python 数据结构。 有关`pprint`的更多信息，请参见](https://docs.python.org/3/library/pprint.html)。
