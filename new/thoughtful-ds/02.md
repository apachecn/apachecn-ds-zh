...
当我们需要构建实时仪表板以可视化结果时，我们在 Twitter 主题标签项目的情绪分析中亲身体验了它。 数据分析是使用熊猫，Apache Spark 和一些绘图库（例如 Matplotlib 和 Bokeh）以 Python 编写的，而仪表板是用 [Node.js](https://nodejs.org) 和 [D3](https://d3js.org) 编写的。
