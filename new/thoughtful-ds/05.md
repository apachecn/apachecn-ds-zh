...
*   **程序化**：在某些情况下，您可能希望通过 JavaScript 直接触发事件。 在这种情况下，您可以使用`pixiedust`全局对象的`sendEvent(payload, divId)`方法。 `divId`是一个可选参数，用于指定事件的来源。 如果省略了·参数，则默认为当前发送事件的元素的 divId。 因此，您应始终使用来自用户事件（例如，点击和悬停）的 JavaScript 处理程序的`pixiedust.sendEvent`，而不使用`divId`。
