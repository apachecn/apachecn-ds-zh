...
*   **行魔术**：语法为`%<magic_command_name> [optional arguments]`，例如`%matplotlib inline`，它将 Matplotlib 配置为在 Notebook 输出单元格中内联输出图表。
...
导入`pixiedust_node`时，会从 Python 端创建一个 Node 子进程，以及一个特殊线程，该线程读取该子进程的输出并将其传递给 Python 端，以在当前在其中执行的单元格中显示。 笔记本。 此子过程负责启动 **REPL** 会话（[*读取-求值-打印-循环**](https://en.wikipedia.org/wiki/Read-eval-print_loop)），它将执行从 Notebook 发送的所有脚本，并使所有创建的类，函数和变量在所有执行中都可重用。
