...
4.  您可能还需要自定义`display_name`键，使其具有唯一性，并且可以从Juptyer UI轻松识别。 如果需要了解现有内核的列表，可以使用以下命令：
...
2.  填写应用程序详细信息，同意开发者协议，然后单击**创建您的Twitter应用程序**按钮。
...
*   CSV文件写在`output_dir`中，该文件在其自己的变量中定义。 在开始时，我们首先删除目录及其内容：
...
*   我们还将在标题中显示当前的推文计数。 为此，我们添加一个`<div>`元素，该元素每5秒刷新一次，并添加一个`<pd_script>`，该元素使用`streams_manager.twitter_stream.listener.tweet_count`来打印当前的tweet计数，该变量是我们添加到 `RawTweetsListener`类。 我们还更新了`on_data()`方法，以在每次有新的tweet到达时增加`tweet_count`变量，如以下代码所示：
