...
在面向对象的分析和编程中，识别对象是一项非常重要的任务。 但这并不总是像我们一直在做的那样简单地在短段中计算名词一样容易。 记住，对象是既具有数据又具有行为的事物。 如果仅处理数据，通常最好将其存储在列表，集合，字典或其他 Python 数据结构中（我们将在第 6 章，*中进行全面介绍 Python 数据结构*）。 另一方面，如果我们仅处理行为，而不处理存储的数据，则更简单的功能是更合适的。
