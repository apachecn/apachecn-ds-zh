...
好吧，至少它抛出了一个有用的例外。 我们将在第 4 章和*期望意外的*中详细介绍异常。 您可能之前已经看过它们（尤其是普遍存在的 **SyntaxError**，意味着您键入的内容不正确！）。 此时，只需注意这意味着出了点问题。
...
此方法已更新为返回`True`或`False`，具体取决于是否已找到便笺。 如果用户输入了无效的注释，菜单可能会使用此返回值显示错误。 这段代码有点笨拙。 如果它引发了异常，看起来会更好一些。 我们将在第 4 章和*中介绍意外的*。
