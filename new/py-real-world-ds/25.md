...
加载文件时，我们将 delimiter 参数设置为制表符，告诉Pandas不要读取第一行作为标题（带有`header=None`），并设置列名。 让我们看下面的代码：
...
我们使用 values 参数获取实际值（而不是当前存储在`title_object`中的Pandas`Series`对象）。 我们只对第一个值感兴趣-无论如何，给定的`MovieID`应该只有一个标题！
