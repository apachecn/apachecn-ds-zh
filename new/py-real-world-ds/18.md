...
该标志将预加载`matplotlib`和`numpy`与默认的 matplotlib 后端进行交互使用。 IPython 可以在各种环境中运行：在终端中作为`Qt`应用程序运行，或者在浏览器内部运行。 这些选项值得探索，因为 IPython 已在许多用例中得到广泛采用，例如原型设计，用于使会议演讲或讲座更具吸引力的交互式幻灯片，以及作为共享研究的工具。
...
Bokeh 是，由 Peter Wang，Hugo Shi 和其他人在 Continuum Analytics 中进行。 它旨在以`D3.js`的样式提供优雅且引人入胜的可视化。 该库可以快速轻松地创建交互式绘图，仪表板和数据应用程序。 这是 matplotlib 和 Bokeh 之间的一些差异：
