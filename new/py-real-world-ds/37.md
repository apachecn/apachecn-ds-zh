...
scikit-learn 的一个不错的功能是它允许我们在训练后将决策树导出为`.dot`文件，我们可以使用 GraphViz 程序将其可视化。 该程序可从 [http://www.graphviz.org](http://www.graphviz.org) 免费获得，并受 Linux，Windows 和 Mac OS X 支持。
...
如果我们将参数`p=2`或曼哈顿距离分别设置为`p=1`，则它成为欧几里德距离。 scikit-learn 中提供了许多其他距离度量，并且可以将提供给`metric`参数。 它们在 [http://scikit-learn.org/stable/modules/genic/sklearn.neighbors.DistanceMetric.html](http://scikit-learn.org/stable/modules/generated/sklearn.neighbors.DistanceMetric.html) 中列出。
