...
接下来，我们分别提取对应于 50 朵*鸢尾花*和 50 朵*鸢尾花*花的前 100 个类别标签，并将类别标签转换为两个整数类别标签[ 我们分配给向量`y`的 HTG0]（ *Versicolor* ）和`-1`（ *Setosa* ），其中Pandas`DataFrame`的值方法产生相应的 NumPy 表示形式。 同样，我们提取这 100 个训练样本的第一特征列（*萼片长度*）和第三特征列（*花瓣长度*），并将它们分配给特征矩阵`X`， 我们可以通过二维散点图进行可视化：
