...
尽管准备数据似乎是一项平凡的任务，而且通常是这样，但这是我们不能跳过的一步，尽管我们可以通过使用诸如Pandas之类的工具来简化数据。
...
`age`列看起来不错，因为Pandas已经推断出了预期的类型，但是`height`尚不能解析为数值：
...
如果我们尝试将`height`列强制转换为浮点值，则Pandas将报告异常：
...
情况很常见：您有多个数据源，但是为了对内容进行陈述，您宁愿将它们组合在一起。 幸运的是，当合并，联接或对齐数据时，Pandas的串联和合并功能消除了大部分麻烦。 它也以高度优化的方式进行。
...
数据帧类似于数据库表。 因此，Pandas对它们执行类似 SQL 的联接操作就不足为奇了。 令人惊讶的是，这些操作是高度优化且极其快速的：
