...
我们在第 7 章，*中发现了要使用图形挖掘进行跟踪的帐户*，即分页如何适用于 Twitter API。 我们将光标与返回的结果一起发送，并随请求发送。 然后，Twitter 将使用此光标来获取结果的下一页。 除了调用参数`after`之外，reddit API 的功能几乎完全相同。 我们第一页不需要它，因此我们最初将其设置为`none`。 我们将在结果的第一页之后将其设置为有意义的值。 代码如下：
...
在 k 均值算法中，使用每个特征时均不考虑其权重。 从本质上讲，所有功能均假定为相同比例。 我们在第 2 章，*用 scikit-learn 估计器*分类中看到了不具有缩放功能的问题。 结果是 k-means 正在寻找圆形簇，如以下屏幕截图所示：
