...
*   **从源**中提取数据：数据可以多种形式出现，例如 Excel，CSV，JSON，数据库等。 在一些有用的软件包的帮助下，Python 使得从源中读取数据变得非常容易，本章将对此进行介绍。
...
要将数据集组合在一起，可以利用熊猫的`concat`功能。 让我们以`Area Name`和`County`列的前五行为例：
...
连接使用`join()`方法进行。 第一个参数采用在其上进行查找的 DataFrame。 请注意，`grade_lookup` DataFrame 的索引是通过`set_index()`方法设置的。 这对于联接至关重要，因为没有联接，联接方法将不知道将 DataFrame 联接到哪一列。
