...
我们将在本章中广泛使用 Python 的**自然语言工具包**（ **NLTK** ）包进行文本挖掘。 您可以在 [http://www.nltk.org/install.html](http://www.nltk.org/install.html) 上的说明帮助下进行安装。
...
您可以通过参考 [https://github.com/amueller/word_cloud](https://github.com/amueller/word_cloud) 遵循说明进行操作。
...
语音标记部分是文本分析的重要任务之一。 它有助于根据句子的上下文或单词在句子中扮演的角色来标记每个单词。

让我们看看如何使用`nltk`执行部分语音标记：
...
您可以在 [https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html](https://www.ling.upenn.edu/courses/Fall_2003/ling001/penn_treebank_pos.html) 上获取有关以上代码中使用的标签的更多信息。
...
词干和词根去除是用于找到这些共同根的技术。 找到根源将有助于我们计算，玩耍，玩耍和玩耍，就像所有单词都在谈论玩耍一样。
...
`WordNet`是 Princeton 的英语词汇数据库，我们将使用其词素化技术：
...
命名实体识别器是，用于将句子的元素分类为类别的任务，例如人，组织，位置等。 斯坦福命名实体识别器是其中最流行的一种，可以在 [http://nlp.stanford.edu/](http://nlp.stanford.edu/) 中找到。

可以在 [http://nlp.stanford.edu/software/stanford-ner-2014-06-16.zip](http://nlp.stanford.edu/software/stanford-ner-2014-06-16.zip) 上下载以斯坦福命名的实体识别器。
...
# 使用 Twitter 对世界领导人进行情感分析

在开始分析推文之前，我们需要安装 Python 的`Twython`软件包，该软件包有助于与 Twitter API 进行交互以从 Twitter 获取推文。 可以从 [https://github.com/ryanmcgrath/twython](https://github.com/ryanmcgrath/twython) 下载。

另外，您需要从 [https://apps.twitter.com/app/new](https://apps.twitter.com/app/new) 获取使用者密钥和使用者秘密密钥。
...
我们将分析来自奥巴马，普京，莫迪，习近平和戴维·卡梅伦的推文的情绪。 以下是我们将要进行分析的一些假设：
...
现在，让我们定义一个函数来对推文的情绪进行评分。 我们在 [http://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html](http://www.cs.uic.edu/~liub/FBS/sentiment-analysis.html) 上提供了来自胡和刘的词典中的正反列表。
