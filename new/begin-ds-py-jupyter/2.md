...
到此结束了有关在机器学习应用中使用的清洗数据的部分。 让我们花一点时间来说明我们的 Jupyter笔记本在测试各种数据转换方法以及最终记录我们决定的管道方面的有效性。 通过在处理之前仅更改特定的代码单元，可以很容易地将应用于数据的更新版本。 另外，如果我们希望对处理进行任何更改，则可以在笔记本中轻松测试这些更改，并且可以更改特定的单元以适应更改。 实现此目的的最佳方法可能是将笔记本复制到新文件中，以便我们始终可以保留原始分析的副本以供参考。
