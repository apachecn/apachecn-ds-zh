...
正如在本书中一次又一次地看到的那样，Pandas是使用 Python 和 Jupyter笔记本进行数据科学不可或缺的一部分。 DataFrames 提供了一种组织和存储带标签数据的方法，但是更重要的是，pandas 提供了节省时间的方法来转换 DataFrame 中的数据。 我们在本书中看到的示例包括删除重复项，将字典映射到列，在列上应用函数以及填写缺失值。
