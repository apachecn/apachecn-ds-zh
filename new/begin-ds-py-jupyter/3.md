...
2.  像scikit-learn一样，Bokeh模块通常是分段加载的（例如，与熊猫不同，一次加载整个库）。 通过运行以下代码导入一些基本的绘图模块：
...
8.  通过运行以下代码，将`'Year of last change'`列添加到DataFrame中：
