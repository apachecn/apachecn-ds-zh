...
*   `shape`的工作方式类似于 NumPy 数组或`pandas`数据帧的常规`shape`属性。 它显示每个尺寸的大小。 由于`array_x`具有`1`尺寸和`3`元素，因此它是具有`3`大小的元组。
...
`output_array`的类型和尺寸应与操作的预期输出相同：
...
[第 5 章](../Text/5.html)和*使用 NumPy* 批发分销商的客户端的一维聚类功能将在此处使用，以下代码段应保存到名为`to_be_profiled.py`的文件中：
