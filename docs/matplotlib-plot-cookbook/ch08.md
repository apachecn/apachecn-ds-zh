...
matplotlib 不仅可以绘制图表 它可以绘制您可以与之交互的图形。 交互式可视化可能是探索一些数据并发现一些有趣模式的好方法。 同样，交互式人物可以为教学目的提供很好的支持。 在本章中，我们将探讨创建 **交互式绘图**所必须使用的不同选项。
...
wxWidgets 用户界面是 Python 的另一个 GUI 模块，它绑定了 wx 库。 wx 库公开了用于在 Windows，OS X 和 Linux 上创建图形界面的通用 API。 用 wx 创建的图形界面将具有运行它们的平台的外观。 在本食谱中，我们将研究如何将 wxWidgets 与 matplotlib 接口。
...
用户界面的外观将根据在哪个平台上运行脚本而有所不同：Linux，Windows，OS X 等。
