...
让我们从最基本方案开始。 我们有一个 2D 数组，我们想可视化它的内容。 例如，我们将可视化 Mandelbrot 集。 Mandelbrot 集（一种著名的分形形状）将多次迭代与平面上的每个点相关联。
...
根据您的计算机，此脚本可能需要几秒钟到几分钟才能产生输出。 减少`N`，即我们正在填充的正方形数组的大小，将减少计算量。 结果将是 Mandelbrot 集的所有分形荣耀的视图：
...
`pyplot.imshow()`功能非常简单； 给它一个 2D 数组，它将渲染一张图片，其中每个像素代表一个从 2D 数组获取的值。 像素的颜色是从颜色图中选取的-数组的每个值都在`[0, 1]`间隔内线性归一化。 `pyplot.imshow()`函数可渲染人物，但不会显示它。 与往常一样，我们应该致电`pyplot.show()`来查看该图。 但是，具有两个具有类似名称的功能可能会引起混淆。

脚本的其余部分生成我们的示例数据。 创建 2D 数组`Z`，然后用双循环填充。 此循环对`[-2.2, 0.8]*[-1.5, 1.5]`正方形进行采样。 对于每个样本，`iter_count`函数都会计算 Mandelbrot 集的迭代次数。 `Z`数组中的数据可能来自文件或任何其他来源。
...
由于我们使用了颜色图，因此我们应该导入 matplotlib 的颜色图模块。 在脚本的开头，添加以下行：
...
`extent` 可选参数指定 2D 数组中存储的数据的坐标系。 坐标系是四个值的元组。 最小和最大范围在水平轴上，然后在垂直轴上。 现在，轴显示我们采样以计算 Mandelbrot 集的正方形的坐标。 参数`cmap`指定颜色图。

现在，让我们在脚本中将样本数据的大小从`512`减小为`32`。 输出将类似于以下屏幕截图：

![There's more…](img/3265OS_06_03.jpg)

我们使用`32*32` 样本而不是`512*512`样本来表示 Mandelbrot 集。 但是的结果并不小。 实际上，`pyplot.imshow()`所做的不只是为 2D 数组着色像素。 如果输入数据小于或大于数字，`pyplot.imshow()`功能将产生给定任意大小的图片并执行插值。 在此示例中，我们可以看到默认插值是线性的。 这并不总是理想的。 `pyplot.imshow()`函数有一个可选参数`interpolation`，它允许我们指定要使用的插值类型。 matplotlib 提供了令人印象深刻的插值方案列表。 让我们看一下最简单的插值方案：最近邻插值：
...
彩色图是产生可读和视觉悦目的图形的关键要素。 但是，我们在这里进行科学，而审美只是一个副目标。 使用颜色图时，我们想知道哪个值对应于给定的颜色。 在本食谱中，我们将探讨一种将此类信息添加到图形的简单方法。

## 怎么做...

我们将使用相同的示例 Mandelbrot 集。 我们只需将调用添加到`pyplot.colorbar()`：
...
整洁的颜色条可让您将颜色表中的颜色与感兴趣的值相关联。 这是 Mandelbrot 迭代计数。
...
该脚本绘制了从与先前配方相同的正方形采样的 Mandelbrot 集。 但是，我们不使用常规的网格采样，而是对 Mandelbrot 集进行随机采样，如下例所示：
...
该脚本将显示随机采样的 Mandelbrot 集。 样本点显示为黑色小点：

![How to do it...](img/3265OS_06_07.jpg)

显然，由于的随机采样过程，结果比常规采样的结果更加混乱。 但是，我们仅使用了 4,096 个样本，而不是先前示例中使用的 262,144 个样本，因此我们得到的结果是令人满意的。 通过 matplotlib 的**非均匀采样**功能，使用自适应采样方法将使您能够以比常规网格采样低得多的计算成本获得 Mandelbrot 集的高分辨率视图。

## 工作原理...

首先，该脚本对 Mandelbrot 集进行随机采样，这由脚本的以下部分完成：
...
注意明智地选择颜色图会有所帮助； 在此，负值显示为黑色，正值显示为白色。 这样，我们就可以一目了然地看到符号和大小信息。 使用从红色到蓝色，中间为白色的颜色图，效果更好。
...
功能`pyplot.contour()`允许您生成轮廓注释。 为了演示，让我们重用先前食谱中的代码，以研究 Mandelbrot 集的放大部分：
...
前面的脚本将显示带有复杂轮廓注释的 Mandelbrot 集的详细信息：

![How to do it...](img/3265OS_06_09.jpg)

## 工作原理...

我们认识到先前在 Mandelbrot 套装中展示的配方中使用的代码。 唯一的区别是，我们通过更改`xmin`，`xmax`，`ymin`和`ymax`的值来放大 Mandelbrot 集的特定细节。 和以前一样，我们使用`pyplot.imshow()`渲染每个样本的迭代计数。

仅添加了一个：调用`pyplot.contour()`。 此函数获取样本网格的坐标`X`和`Y`以及存储在矩阵`Z`中的样本。 然后，该功能将渲染与`levels`列表中指定的值相对应的轮廓。 可以使用可选参数`cmap`使用色图对级别进行着色。 我们可以使用可选参数`color`为所有轮廓指定一种唯一的颜色。

每个轮廓的水平可以用色条显示或直接在图形上显示。 `pyplot.contour()`函数返回`Contour`实例。 `pyplot.clabel()`函数采用`Contour`实例和一个可选的格式字符串来渲染每个轮廓的标签。

## 还有更多...

在此，轮廓仅显示为线条。 但是，我们可以显示填充轮廓。 让我们在之前使用的 Mandelbrot 集的相同细节上进行演示：
...
使用箭头指向表示矢量场效果很好。 但是 matplotlib 可以做得更好，它可以显示矢量场的流线。 一条流线显示了矢量场的流动方式。 在本食谱中，我们将向您展示如何创建流线型。
...
我们可以使用带有可选参数`color`和`cmap`的颜色图为流线着色：
