...
`pyplot.imshow()`功能非常简单； 给它一个 2D 数组，它将渲染一张图片，其中每个像素代表一个从 2D 数组获取的值。 像素的颜色是从颜色表中选取的-数组的每个值都在`[0, 1]`间隔内线性归一化。 `pyplot.imshow()`函数可渲染人物，但不会显示它。 与往常一样，我们应该致电`pyplot.show()`来查看该图。 但是，具有两个具有类似名称的功能可能会引起混淆。
...
`extent` 可选参数指定 2D 数组中存储的数据的坐标系。 坐标系是四个值的元组。 最小和最大范围在水平轴上，然后在垂直轴上。 现在，轴显示我们采样以计算 Mandelbrot 集的正方形的坐标。 参数`cmap`指定颜色表。
