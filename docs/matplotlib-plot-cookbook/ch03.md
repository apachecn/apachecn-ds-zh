...
使您的图表不言自明是一种很好的做法。 但是，很难在没有任何注释的情况下使一些曲线和点不言自明。 人们应该如何读取垂直和水平轴？ 该框和该曲线代表哪个数量？ matplotlib 提供了很多注释图形的可能性，我们将在本章中进行探讨。
...
现在我们可以注释数字。 但是，在科学和工程方面，以前演示的解决方案受到一个令人讨厌的限制。 我们不能使用数学符号！ 或者，可以吗？ 在本秘籍中，我们将了解如何使用 LaTeX 在图中显示和数学脚本。
...
标题后，图形轴的正确描述有助于用户理解图形。 在本秘籍中，我们将向您展示如何在图形的每个轴旁边获取标签。
...
该图将与本章第一个秘籍中获得的图相同。 但是，两个轴都具有图例。
...
添加文本框可以帮助您注释图形。 但是，要显示图片的特定部分，使用箭头没有什么可比的。 在本秘籍中，我们将向您展示如何在图形上添加箭头。
...
每个`pyplot`函数都有可选的`label`参数，以命名元素，例如图形的曲线，直方图等。 matplotlib 跟踪这些标签。 `pyplot.legend()`功能将渲染图例。 图例是从标签自动生成的。
...
需要使用`pyplot.axis('scaled')`功能以确保图形使用统一的比例尺：与 *x* 和 *y* 轴上使用的比例尺相同。 这与默认行为`'tight'`形成对比，其中默认值 matplotlib 将为 *x* 和 *y* 轴赋予不同的比例，以使图形尽可能紧密地适合显示表面 。 此功能将在第 4 章，“处理图形”中引入。
...
在 matplotlib 中，刻度在图形的两个轴上都是小标记。 到目前为止，我们让 matplotlib 处理轴图例上刻度线的位置。 正如我们将在本秘籍中看到的那样，我们可以手动覆盖此机制。
