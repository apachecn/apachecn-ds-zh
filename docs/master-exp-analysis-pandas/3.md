...
由于它是 CSV 文件，因此我们正在使用 Pandas 的`read_csv`函数。 现在我们已经将数据集读入了 DataFrame 中，让我们看一些记录：
...
本节重点介绍如何使用 Pandas`merge()`和`concat()`方法组合两个或多个 DataFrame。 我们还将探讨`merge()`方法以各种方式加入 DataFrame 的用法。
