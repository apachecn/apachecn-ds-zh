...
由于它是 CSV 文件，因此我们正在使用熊猫的`read_csv`函数。 现在我们已经将数据集读入了 DataFrame 中，让我们看一些记录：
...
在这里，我们将其中一列设置为 DataFrame 的索引。 我们可以看到索引已在内存中设置。 现在，让我们检查一下它是否已修改原始 DataFrame：
...
我们可以看到在原始 DataFrame 中没有任何变化。 `set_index`方法仅在内存中全新的 DataFrame 中创建了更改，我们可以将其保存在新的 DataFrame 中。 现在让我们看看如果传递`inplace`参数，它将如何工作：

```py
top_movies.set_index('Rank', inplace=True)
```

我们将`inplace=True`传递给该方法。 现在让我们检查原始的 DataFrame：
...
我们可以看到传递`inplace=True`确实修改了原始 DataFrame。 并非所有方法都需要使用`inplace`参数来修改原始 DataFrame。 例如，`rename(columns)`方法修改原始的 DataFrame，而不需要`inplace`参数：
...
让我们先问一个问题，看看熊猫的`groupby`方法是否可以帮助我们获得答案。 我们想要获取每个`State`的平均值`Price`值：
...
该数据集是泰坦尼克号的乘客生存数据集，可从 Kaggle 的 [https://www.kaggle.com/c/titanic/data](https://www.kaggle.com/c/titanic/data) 下载。

让我们看看首先丢失了多少条记录。 为此，我们首先需要找出数据集中的总 记录数。 我们通过在 DataFrame 上调用 `shape` 属性来做到这一点：
...
然后，我们找出每一列中的记录数。 我们可以通过在 DataFrame 上调用`count`方法来做到这一点：
...
当运行此放置行方法时，我们将结果分配回新的 DataFrame 中。 在`891.`总数中，仅剩下`183`记录，但是，这可能会导致丢失大量数据，并且可能无法接受。
...
`index_col`参数采用单个数字值或一系列值。 在这里，我们传递`Name`列的索引。

接下来，让我们看看如何使用索引进行数据选择。 在以下屏幕截图中，我们在 DataFrame 上调用`loc`方法，并传入我们要选择的记录的索引级别：
...
我们可以通过几种方法来重命名 pandas DataFrame 中的列。 一种方法是在从数据集中读取数据时重命名列。 为此，我们需要将列名作为列表传递给`read_csv`方法的`names`参数：
...
现在，我们在 DataFrame 上调用`rename`方法，并将列名（旧值和新值）传递给`columns`参数：
...
要从我们的数据集中删除单个列，请使用 pandas `drop()`方法。 `drop()`方法由两个参数组成。 第一个参数是需要删除的列的名称； 第二个参数是`axis`。 此参数告诉`drop`方法是否应该删除行或列，并将`inplace`设置为`True`，这告诉该方法将其从原始 DataFrame 本身删除。
...
对于此示例，让我们创建自己的 DataFrame 数据集。 我们可以使用以下代码来做到这一点：
...
在继续选择和过滤`datetime`系列之前，我们需要确保为`DOB`列设置了索引。 为此，我们使用以下代码：
...
我们还可以使用时间序列属性来最有效地利用`datetime`系列数据。 使用此功能的缺点是`datetime`字段必须是列，而不是行。
...
与往常一样，我们首先将`pandas`模块导入到 Jupyter Notebook 中，如下所示：
...
不建议使用此解决方案，因为这可能会影响我们的运营结果。 解决此警告的另一种方法是使用`is_copy`方法。 在这里，我们创建 DataFrame 的新副本并将`is_copy`设置为`None`，如下所示：
...
首先，将`pandas`模块导入 Jupyter Notebook：
...
让我们继续使用熊猫的`apply`方法来应用函数。 在此示例中，我们将使用`lambda`创建一个函数，如下所示：
...
如果仔细观察，`Name`字段中的值已转换为小写。 接下来，我们了解如何将函数应用于多个列或整个 DataFrame 中的值。 我们可以使用`applymap()`方法。 它以类似于`apply()`方法的方式工作，但是在多列或整个 DataFrame 上。 以下代码描述了如何将`applymap()`方法应用于`Age`和`Pclass`列：
...
让我们继续学习有关将多个 DataFrame 合并和串联在一起的知识。

# 将多个 DataFrame 合并并连接成一个

本节重点介绍如何使用熊猫`merge()`和`concat()`方法组合两个或多个 DataFrame。 我们还将探讨`merge()`方法以各种方式加入 DataFrame 的用法。
...
在此示例中，让我们将这两个 DataFrame 垂直放置在一起。 使用 pandas `concat()`方法通过传递两个 DataFrame 作为其参数来执行此操作：
...
为了对这些数据集执行内部合并，我们将 DataFrames 传递给`merge()`方法。 我们还指定必须在其上进行合并的列，同时确保我们指定它是内部合并。 您的数据集应类似于下表：
...
现在，这意味着我们将两个数据集中的数据放在一起。 它仅包含在两个 DataFrame 中具有通用标签的那些行。 接下来，我们进行外部合并。 这是通过将`how`参数作为`left`传递给`merge()`方法来完成的：
