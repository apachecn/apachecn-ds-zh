...
序列和数据帧都具有强大的功能。 我们可以使用`dir`函数来揭示系列的所有属性和方法。 此外，我们可以找到序列和数据帧共有的属性和方法的数量。 这两个对象共享绝大多数的属性和方法名称：
...
为什么 Pandas 提供与这些运算符等效的方法？ 从本质上讲，运算符只能以一种方式进行操作。 另一方面，方法可以具有允许您更改其默认功能的参数：
...
步骤 2 将四个不同的序列与 plus 运算符相加。 步骤 3 使用方法链来查找和填充缺失值。 步骤 4 使用大于或等于比较运算符返回布尔系列，然后在步骤 5 中使用`all`方法对其进行求值，以检查每个单个值是否为`True`。
