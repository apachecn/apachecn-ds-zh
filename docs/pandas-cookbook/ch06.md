...
如第 1 章， *Pandas Foundations* 中所讨论的，序列和数据帧的每个轴都有一个索引对象，用于标记值。 有许多不同类型的索引对象，但是它们都具有相同的共同行为。 除特殊的 MultiIndex 之外，所有索引对象都是一维数据结构，结合了 Python 集和 NumPy ndarrays 的功能和实现。
