...
由 Axes 对象创建的所有这些有用的绘图元素都称为 **artist** 。 甚至 Figure 和 Axes 对象本身也是艺术家。 对艺术家的这种区分对本秘籍而言并不重要，但在进行更高级的 matplotlib 绘图时，尤其是在阅读文档时，将很有用。
...
不幸的是，同时使用这两个选项会导致很多混乱，并且 matplotlib 以难以学习而著称。 该文档提供了使用这两种方法的示例。 教程，博客文章和 Stack Overflow 文章在网络上比比皆是，这使这种混乱永久化。 本秘籍仅专注于面向对象的方法，因为它具有更多的 Python 风格，并且与我们与 Pandas 互动的方式更加相似。
...
默认情况下，两变量线图和散点图使用索引作为 *x* 轴，将列的值用作 *y* 轴。 单变量图忽略索引，并对每个变量应用转换或聚合以制作其图。 在本秘籍中，我们将考察 Pandas 中两变量和一变量地块之间的差异。
...
在本秘籍中，我们将研究性别和种族之间的经验年限与薪水之间的关系。 我们将首先使用海洋轴函数创建一个简单的回归图，然后使用 Grid 函数为该图添加更多尺寸。
