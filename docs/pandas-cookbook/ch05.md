...
所有三个条件都必须为`True`以匹配配方要求。 它们每个都与和号字符`&`组合在一起，后者在每个序列值之间创建逻辑`and`条件。
...
对于许多操作，大 Pandas 有多种方法来做同一件事。 在前面的配方中，薪水标准使用两个单独的布尔表达式。 与 SQL 相似，序列具有`between`方法，其工资标准等效编写如下：
...
7.  事实证明，几列没有相同的数据类型。 Pandas 对于这些情况有另一种选择。 在其开发人员主要使用的测试模块中，有一个函数`assert_frame_equal`，您可以使用它检查序列和 DataFrames 的相等性，而无需同时检查数据类型的相等性：
...
在数据分析过程中，持续验证结果非常重要。 检查序列和 DataFrames 的相等性是一种非常通用的验证方法。 我们在步骤 4 中的首次尝试产生了意外结果。 在深入研究之前，一些基本的健全性检查（例如确保行和列的数目相同或行和列的名称相同）是很好的检查。

步骤 6 将两个系列的数据类型一起比较。 在这里，我们揭示了 DataFrames 不等效的原因。 `equals`方法检查值和数据类型是否相同。 步骤 7 中的`assert_frame_equal`函数具有许多可用参数，可以通过各种方式测试相等性。 注意，调用`assert_frame_equal`后没有输出。 当两个传递的数据帧相等时，此方法返回 None；否则，将引发错误。
