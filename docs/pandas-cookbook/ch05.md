...
从数据集中过滤数据是最常见的基本操作之一。 有许多方法可以使用 **b** **oolean indexing** 过滤（或子集）Pandas 中的数据。 布尔索引（也称为**布尔选择**）可能是一个令人困惑的术语，但出于 Pandas 的目的，它是指通过为每行提供布尔值（`True`或`False`）来选择行 。 这些布尔值通常存储在 Series 或 NumPy `ndarray`中，通常是通过将布尔条件应用于 DataFrame 中的一个或多个列来创建的。 我们首先创建布尔系列并计算它们的统计量，然后继续创建更复杂的条件，然后以多种方式使用布尔索引来过滤数据。
...
*   一起参考*连锁系列方法[第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)，*Pandas 基金会*的*配方
*   请参阅*与操作员一起使用[第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)，*Pandas 基金会*中的*配方
...
在 SQL SELECT 语句中，WHERE 子句非常常见，并过滤数据。 此食谱将编写与选择雇员数据集的特定子集的 SQL 查询等效的 Pandas 代码。
...
对于许多操作，大 Pandas 有多种方法来做同一件事。 在前面的配方中，薪水标准使用两个单独的布尔表达式。 与 SQL 相似，Series 具有`between`方法，其工资标准等效编写如下：
...
*   请参阅[第 9 章](../Text/ch09.html#EBLF40-5686c430c0a9442a82c4c5795c8553c9)和*合并 Pandas 对象*中的*连接到 SQL 数据库配方*。
...
*   Pandas`pct_change`系列方法的官方文档（ [http://bit.ly/2wcjmqT](http://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.pct_change.html) ）
...
传递给`query`方法的字符串看起来比普通的 Pandas 代码更像普通的英语。 与`depts`一样，可以使用 at 符号（`@`）来引用 Python 变量。 通过简单地引用其名称而不用内引号，可在查询名称空间中使用所有 DataFrame 列名称。 如果需要一个字符串，例如`Female`，则需要用引号将其引起来。
...
Pandas 实际上具有复制此操作的内置方法`clip`，`clip_lower`和`clip_upper`。 `clip`方法可以同时设置地板和天花板。 我们还检查此替代方法是否产生完全相同的 Series，它会执行以下操作：
...
7.  事实证明，几列没有相同的数据类型。 Pandas 对于这些情况有另一种选择。 在其开发人员主要使用的测试模块中，有一个函数`assert_frame_equal`，您可以使用它检查 Series 和 DataFrames 的相等性，而无需同时检查数据类型的相等性：
