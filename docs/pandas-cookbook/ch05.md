...
*   一起参考*连锁系列方法第 1 章，*Pandas 基金会*的*秘籍
*   请参阅*与操作员一起使用第 1 章，*Pandas 基金会*中的*秘籍
...
*   请参阅第 11 章和*使用 Matplotlib，Pandas 和 Seaborn* 进行可视化
...
*   请参阅第 9 章和*合并 Pandas 对象*中的*连接到 SQL 数据库秘籍*。
...
第 4 章和*选择数据子集*涵盖了有关通过`.iloc`和`.loc`索引器选择不同数据子集的各种方法。 这两个索引器都通过整数位置或标签同时选择行和列。 这两个索引器都可以通过布尔索引进行数据选择，即使布尔不是整数也不是标签。
