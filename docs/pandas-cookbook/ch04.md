...
Series 或 DataFrame 中数据的每个维度都通过 Index 对象标记。 正是这个索引将 Pandas 数据结构与 NumPy 的 n 维数组分开。 索引为数据的每一行和每一列提供了有意义的标签，而 Pandas 用户可以通过使用这些标签来选择数据。 此外，pandas 允许其用户通过行和列的整数位置选择数据。 这种双重选择功能（一种使用标签，另一种使用整数位置）使得强大而又令人困惑的语法可以选择数据子集。
...
将标量值，标量列表或切片对象传递给`.iloc`或`.loc`索引器，会使 Pandas 扫描索引标签中的适当行并返回它们。 如果传递单个标量值，则返回一个 Series。 如果传递了列表或切片对象，则返回一个 DataFrame。
...
本章前面的食谱展示了如何使用`.iloc`和`.loc`索引器选择任一维度中的 Series 和 DataFrames 的子集。 选择行的快捷方式仅包含索引运算符本身。 这只是显示 Pandas 其他功能的捷径，但索引运算符的主要功能实际上是选择 DataFrame 列。 如果要选择行，则最好使用`.iloc`或`.loc`，因为它们是明确的。
