...
本章介绍了数据帧的许多基本操作。 许多秘籍将与第 1 章， *Pandas Foundations* 中的内容类似，这些内容主要涵盖系列操作。

# 选择多个数据帧列

选择单个列是通过将所需的列名作为字符串传递给数据帧的索引运算符来完成的。 在第 1 章，*Pandas “Pandas 基础”*的*选择系列*秘籍中对此进行了介绍。 通常需要关注当前工作数据集的一个子集，这是通过选择多个列来完成的。
...
您需要熟悉所有 Pandas 数据类型以及如何访问它们。 第 1 章， *Pandas Foundations* 中的*了解数据类型*秘籍具有包含所有 Pandas 数据类型的表。
...
*   请参阅*了解数据类型。第 1 章， *Pandas Foundations**
...
在第 1 章， *Pandas Foundations* 的*调用系列方法*秘籍中，对单列或系列数据进行操作的各种方法。 当从数据帧调用这些相同的方法时，它们会立即对每一列执行该操作。
...
无论您相信方法链接是否是一种好的做法，在使用 Pandas 进行数据分析时都会遇到它是很普遍的。 *连锁系列方法结合在一起第 1 章， *Pandas Foundations* 中的*秘籍展示了连锁系列方法一起的几个示例。 本章中的所有方法链都将从数据帧开始。 方法链接的关键之一是知道在链接的每个步骤中返回的确切对象。 在 Pandas 中，这几乎总是一个数据帧，序列或标量值。
...
*中提供了关于运算符的入门知识，它与第 1 章，*Pandas “Pandas 基础”*的* 系列秘籍秘籍有关。 这里。 Python 算术和比较运算符直接在数据帧上工作，就像在序列上一样。
...
步骤 1 将一个数据帧与一个标量值进行比较，而步骤 2 将一个数据帧与另一个数据帧进行比较。 乍看之下，这两种操作都非常简单直观。 第二个操作实际上是检查数据帧是否具有相同标签的索引，以及是否具有相同数量的元素。 如果不是这种情况，操作将失败。 有关更多信息，请参见第 6 章，“索引对齐”中的*生产笛卡尔积*秘籍。
