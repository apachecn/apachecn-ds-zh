...
本章介绍了数据帧的许多基本操作。 许多秘籍将与第 1 章和 *Pandas Foundations* 中的内容类似，这些内容主要涵盖系列操作。

# 选择多个数据帧列

选择单个列是通过将所需的列名作为字符串传递给数据帧的索引运算符来完成的。 在第 1 章，*Pandas 基金会*的*选择系列*秘籍中对此进行了介绍。 通常需要关注当前工作数据集的一个子集，这是通过选择多个列来完成的。
...
*中提供了关于运算符的入门知识，它与第 1 章，*Pandas 基金会*的* 系列秘籍秘籍有关。 这里。 Python 算术和比较运算符直接在数据帧上工作，就像在序列上一样。
