...
选择单个列是通过将所需的列名作为字符串传递给 DataFrame 的索引运算符来完成的。 在[第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)，*熊猫基金会*的*选择系列*配方中对此进行了介绍。 通常需要关注当前工作数据集的一个子集，这是通过选择多个列来完成的。
...
`KeyError`是处理熊猫的最常见例外之一。 此错误主要是由于列名或索引名的错误输入。 每当尝试不使用列表进行多列选择时，都会引发相同的错误：
...
熊猫正试图找到与元组`('actor_1_name', 'actor_2_name', 'actor_3_name', 'director_name')`完全相同的列名。 它失败并引发`KeyError`。
...
您需要熟悉所有熊猫数据类型以及如何访问它们。 [第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)， *Pandas Foundations* 中的*了解数据类型*配方具有包含所有熊猫数据类型的表。
...
`select_dtypes`的一个令人困惑的方面是它同时接受字符串和 Python 对象的灵活性。 下表应阐明选择许多不同列数据类型的所有可能方法。 在熊猫中没有引用数据类型的标准或首选方法，因此最好同时了解两种方式：
...
| `pd.Categorical` | `category` | 熊猫特有的； 没有等效的 NumPy |
...
要从 DataFrame 中选择列的子集，请使用特定列名称的列表。 例如，`movie[['movie_title', 'director_name']]`仅使用`movie_title`和`director_name`列创建一个新的 DataFrame。 通过名称选择列是熊猫 DataFrame 的索引运算符的默认行为。
...
数字列也缺少值，但返回了结果。 默认情况下，pandas 通过跳过数值列来处理缺失值。 通过将`skipna`参数设置为`False`可以更改此行为。 如果存在至少一个缺失值，这将导致所有这些聚合方法的大熊猫返回`NaN`。
...
无论您相信方法链接是否是一种好的做法，在使用熊猫进行数据分析时都会遇到它是很普遍的。 *连锁系列方法结合在一起[第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)， *Pandas Foundations* 中的*配方展示了连锁系列方法一起的几个示例。 本章中的所有方法链都将从 DataFrame 开始。 方法链接的关键之一是知道在链接的每个步骤中返回的确切对象。 在熊猫中，这几乎总是一个 DataFrame，Series 或标量值。
...
为了迫使大熊猫为每一列返回值，我们必须填写缺失值。 在这里，我们选择一个空字符串：
...
*中提供了关于运算符的入门知识，它与[第 1 章](../Text/ch01.html#VF2I0-5686c430c0a9442a82c4c5795c8553c9)，*熊猫基金会*的* 系列配方配方有关。 这里。 Python 算术和比较运算符直接在 DataFrame 上工作，就像在 Series 上一样。
...
熊猫使用 NumPy NaN（`np.nan`）对象表示缺失值。 这是不寻常的对象，因为它不等于其自身。 与自身相比，甚至 Python 的`None`对象也将其评估为`True`：
...
操作的方向是大熊猫中比较混乱的方面之一，互联网上到处都有讨论它的解释的线程。 许多新手熊猫用户很难记住`axis`参数的含义。 幸运的是，在熊猫中，一项操作可以完成两个潜在的方向。 一种可能的方法是尝试双向尝试直到获得所需结果的简单蛮力解决方案。 我记得`axis`参数的含义，认为 1 看起来像一列，对`axis=1`的任何操作都会返回一个新的数据列（与该列具有相同数量的项）。
