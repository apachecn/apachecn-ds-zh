...
在`pandas.testing`子软件包中，存在开发人员在创建单元测试时必须使用的函数。 如果两个数据帧不相等，则`assert_frame_equal`函数将引发`AssertionError`。 如果传递的两个帧相等，则返回`None`：
...
2.  这些大学中许多都缺少其种族列的值。 我们可以计算每一行的所有缺失值，并对所得的系列从最高到最低进行排序。 这将向大学揭示缺少值：
