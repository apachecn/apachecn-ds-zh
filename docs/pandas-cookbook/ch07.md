...
当使用`agg`方法对多个列执行聚合时，[pandas](https://www.interviewbit.com/blog/pandas-vs-numpy/) 将创建一个具有两个级别的索引对象。 聚合列变为顶层，聚合函数变为底层。 Pandas 显示的多重索引级别与单级别的列不同。 除了最里面的**级别**以外，屏幕上不会显示重复的索引值。 您可以检查第 1 步中的数据帧以进行验证。 例如，`DIST`列仅显示一次，但它引用了前两列。
