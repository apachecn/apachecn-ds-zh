...
2.  我们定义了变量并为其分配了一个值。 在此 Python 代码中，变量的类型不固定。 我们可以将变量放入一个列表中，该列表是对应于值的有序序列的内置 Python 类型。 为`var`分配一个列表，如下所示：
...
5.  在此示例中，找到了 Python 2.7。 如果找到 Python，请单击“下一步”。 否则，请单击“取消”并安装Python（如果没有 Python，则无法安装 NumPy）。 点击**下一步**。 这是无可挽回的地方。 很好，但是最好确保要安装到正确的目录，依此类推。 现在开始真正的安装。 可能还要等一下。
