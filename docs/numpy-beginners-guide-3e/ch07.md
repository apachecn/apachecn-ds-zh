...
*   `fv()`函数计算出所谓的**未来值**。 未来值基于某些假设，给出了金融工具在未来日期的价值。
...
未来值根据某些假设给出了金融工具在未来日期的价值。 终值取决于四个参数-利率，周期数，定期付款和当前值。
...
当前值是今天的资产价值。 NumPy `pv()`函数可以计算当前值。 此功能与`fv()`函数类似，并且需要利率，期间数和定期还款，但是这里我们从终值开始。
...
窗口函数是信号处理中常用的数学函数。 应用包括光谱分析和过滤器设计。 这些函数在指定域之外定义为 0。 NumPy 具有许多窗口函数：`bartlett()`，`blackman()`，`hamming()`，`hanning()`和`kaiser()`。 您可以在第 4 章，“便捷功能”和第 3 章，“熟悉常用功能”。
