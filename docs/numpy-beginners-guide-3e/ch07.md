...
*   `argmin()` 和`nanargmin()`函数提供相似的功能  ，但将  的最小值作为功能。 `argmax()`和  `nanargmax()`函数也可用作`ndarray`类的方法。
...
*   `searchsorted()`函数  告诉您数组中的  索引，指定值所属的数组将保持排序顺序。 它使用[二分搜索](https://www.khanacademy.org/computing/computer-science/algorithms/binary-search/a/binary-search)，即`O(log n)`算法。 我们很快就会看到此功能的作用。
