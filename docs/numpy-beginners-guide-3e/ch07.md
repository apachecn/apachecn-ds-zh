...
NumPy `blackman()`函数返回 Blackman 窗口。 唯一参数是输出窗口中`M`的点数。 如果该数字为`0`或小于`0`，则该函数返回一个空数组。
...
我们从样本数据文件中绘制了 APL 的收盘价，该价格使用 Blackum 窗口和 NumPy `blackman()`函数进行了平滑处理（请参见`plot_blackman.py`）：
...
我们用 NumPy `i0()`函数绘制了第一种零阶修改后的 Bessel 函数。
