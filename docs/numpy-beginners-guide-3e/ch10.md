...
我们将使用`matplotlib`下载两个追踪器的去年股票报价。 如先前的第 9 章，“matplotlib 绘图”，我们可以从 Yahoo Finance 检索报价。 我们将比较`DIA`和`SPY`的收盘价的对数回报（DIA 跟踪道琼斯指数； SPY 跟踪 S&P 500 指数）。 我们还将对返回值的差异执行 Jarque–Bera 测试。
...
我们比较了 DIA 和 SPY 的对数回报样本。 另外，我们对日志返回值的差进行了 Jarque-Bera 测试（请参见`pair.py`）：
