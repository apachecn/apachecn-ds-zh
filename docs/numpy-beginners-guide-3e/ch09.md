...
本章是关于`matplotlib`的-Python 绘图库。 我们涵盖了简单图，直方图，图定制，子图，三维图，等高线图和对数图。 您还看到了一些显示股票走势图的示例。 显然，我们只是刮擦了表面，只是看到了冰山的一角。 `matplotlib`的功能非常丰富，因此我们没有足够的空间来覆盖 Latex 支持，极坐标支持和其他功能。
