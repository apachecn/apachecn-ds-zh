...
NumPy 数组通常是同质的（*实战时间 – 创建记录数据类型*部分中介绍了一种异类的特殊数组类型）—数组中的项目 必须是同一类型。 好处是，如果我们知道数组中的项目属于同一类型，则很容易确定数组所需的存储大小。
...
包括**字符代码**是为了与数字向后兼容。 数字是 NumPy 的前身。 不建议使用  ，但此处提供  代码，因为它们会在多个位置弹出。 相反，我们应该使用`dtype`对象。 下表显示了字符代码：
...
Python 类具有函数，如果它们属于一个类，则这些函数称为**方法**。 其中某些方法  是特殊的，  用于创建新对象。 这些专门方法称为**构造函数**。
...
2.  **展开**：适当的  命名函数`flatten()`与`ravel()`相同，但`flatten()`总是分配新的内存，而`ravel()`可能会返回 数组的视图。 视图是共享数组的一种方法，但是您需要对视图小心  ，因为修改视图会影响基础数组，因此会影响其他视图。 数组副本更安全； 但是，它使用更多的内存：
...
3.  **深度堆叠**：另外，使用`dstack()`和元组的深度堆叠堆叠了列表  沿第三个轴（深度）的数组。 例如，将图像数据的二维数组彼此堆叠在一起：
...
4.  **Column stacking**: Stack the one-dimensional arrays with the `column_stack()` function column-wise as follows:
    ...
    `==`运算符用于  Python 与  比较是否相等。 当应用于 NumPy 数组时，运算符将执行逐元素比较。 有关  Python 比较运算符的更多信息，请查看[这里](http://www.pythonlearn.com/html-009/book004.html)。
