...
*   要编译代码，您的操作系统需要一个  编译器。 进行设置超出了本书的范围。 有关在  Windows 上编译 Pygame 的更多信息，可以在[这个页面](http://pygame.org/wiki/CompileWindows)上找到。 有关在  Mac OS X 上编译 Pygame 的更多信息，请参考[这里](http://pygame.org/wiki/MacCompile)。
