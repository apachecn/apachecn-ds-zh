...
对于本教程，您可能需要阅读[有关**勾股三元组**的维基百科页面](http://en.wikipedia.org/wiki/Pythagorean_triple)。 毕达哥拉斯三元组是一组三个自然数，即< b < c，为此， ![Finding Pythagorean triples](img/0945OS_06_04.jpg)。
...
我们对 NumPy 数组应用了随机的  遮罩。 这具有忽略对应于掩码的数据的效果。 您可以在`numpy.ma` 模块中找到一系列掩码数组操作  。 在本教程中，我们仅演示了如何创建掩码数组。
