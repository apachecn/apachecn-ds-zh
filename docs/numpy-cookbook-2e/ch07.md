...
我们测量了`sort()` NumPy 函数的平均  运行时间。 此配方中使用了  以下功能：
...
`line_profiler`由 NumPy 的开发人员之一创建。 此模块对  Python 代码进行逐行分析。 我们将在此食谱中描述必要的安装步骤。

## 准备

您可能需要安装`setuptools`。 先前的食谱中对此进行了介绍； 如有必要，请参阅 *。另请参阅* 部分。 为了安装开发版本，您将需要 Git。 安装 Git 超出了本书的范围。
