...
*   Here are two ways of installing this package on Linux:
    ...
    Linux 软件包的名称也为`pyflakes`。 例如，在 Red Hat 上执行以下操作：
...
要检查 NumPy 代码中的  ，我们需要 Git。 安装 Git 超出了本书的范围（请参见 [http://git-scm.com/book/en/v2/Getting-Started-Installing-Git](http://git-scm.com/book/en/v2/Getting-Started-Installing-Git) ）：
...
1.  The Git command to retrieve the code is as follows:
    ...
    或者，从 [https://github.com/numpy/numpy](https://github.com/numpy/numpy) 下载源档案。
...
**Pylint** 是  另一个由 Logilab 创建的开源静态分析器  。 Pylint 比 Pyflakes 更复杂； 它允许更多的自定义和代码检查。 但是，它比  Pyflakes 慢。 有关更多信息，请参见 [http://www.logilab.org/card/pylint_manual](http://www.logilab.org/card/pylint_manual) 上的手册。

在本食谱中，我们再次从 Git 存储库下载 NumPy 代码-为简便起见，省略了此步骤。
...
**Pychecker** 是  一个古老的静态分析工具。 它不是  十分活跃的开发工具，但它在此提到的速度又足够好。 在编写本书时，最新版本是 0.8.19，最近一次更新是在 2011 年。Pychecker 尝试导入每个模块并对其进行处理。 然后，它搜索诸如传递不正确数量的参数，使用不存在的方法传递不正确的格式字符串以及其他问题之类的问题。 在本食谱中，我们将再次分析代码，但是这次使用 Pychecker。
...
**测试驱动开发** （ **TDD** ）是本世纪软件开发  发生的最好的事情。 TDD 的最重要方面之一是  ，几乎把重点放在单元测试上。
...
1.  Start by writing the following `factorial()` function:
    ...
    该代码与前面的食谱中的代码相同，但是我们添加了一些边界条件检查。
...
我们看到了如何使用标准`unittest` Python 模块实现简单的单元测试。 我们编写了一个测试类  ，该类从`unittest`模块扩展了`TestCase`类。 以下功能用于执行各种测试：
...
在此配方中，我们将测试一个核反应堆，当然不是真正的反应堆！ 此类核反应堆执行阶乘计算，从理论上讲，它可能导致连锁反应，进而导致核灾难。 我们将使用`mock`包通过模拟来模拟阶乘计算。
...
我们将一个字符串传递给`factorial()`方法，以显示带有模拟的代码不会执行实际的代码。 该单元测试的工作方式与上一食谱中的单元测试相同。 这里的第二项测试不测试任何内容。 第二个测试的目的只是演示  如果我们在没有  模拟的情况下执行真实代码会发生什么。
...
**BDD** （ **行为驱动开发** ）是您可能遇到的另一个热门缩写。 在 BDD 中，我们  首先根据某些约定和规则定义（英语）被测系统的预期行为。 在本食谱中，我们将看到这些约定的示例。

这种方法背后的想法是，我们可以让可能无法编程或编写测试大部分内容的人员参加。 这些人编写的功能采用句子的形式，包括多个步骤。 每个步骤或多或少都是我们可以编写的单元测试，例如，使用 NumPy。 有许多 Python BDD 框架。 在本食谱中，我们使用  **生菜** 来测试阶乘功能。
...
3.  提出业务需求是一项艰巨的工作。 以易于测试的方式将其全部写下来更加困难。 幸运的是，这些配方的要求非常简单-我们只需写下不同的输入值和预期的  输出。 我们在`Given`，`When`和`Then`部分中有不同的方案，它们对应于不同的测试步骤。 为阶乘功能定义以下三种方案：
...
我们定义了具有三个方案和相应步骤的功能。 我们使用 NumPy 的测试功能来测试  的不同步骤，并使用`fromstring()`功能从规格文本创建 NumPy 数组。

## 另请参见

*   [http://lettuce.it/](http://lettuce.it/) 上的生菜  文档