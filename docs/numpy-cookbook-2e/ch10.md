...
您需要安装 SciPy 和 NumPy。 返回[第 1 章](../Text/ch01.html "Chapter 1. Winding Along with IPython")， *和 IPython* 一起缠绕，以获取必要的说明。
...
*   **Installing with** `easy_install`: Type any one of the following commands at the command line:
    ...
    由于权限问题，此  可能无法工作，因此您可能需要在命令前面编写`sudo`，或以管理员身份登录。
...
scikit-learn 项目附带了许多我们可以尝试的数据集和样本图像。 在本食谱中，我们将加载 scikit-learn 分发中包含的示例数据集。 数据集将数据保存为 NumPy 二维数组，并将元数据链接到该数据。

## 如何执行...

我们将加载波士顿房价样本数据集。 这是一个很小的数据集，因此，如果您要在  波士顿寻找房子，请不要太兴奋！ 其他数据集在 [http://scikit-learn.org/dev/modules/classes.html#module-sklearn.datasets](http://scikit-learn.org/dev/modules/classes.html#module-sklearn.datasets) 中进行了描述。

我们将查看原始数据的形状及其最大值和最小值。 形状是一个元组，表示 NumPy 数组的尺寸。 我们将对目标数组执行相同的操作，其中  包含作为学习目标（确定房价）的值。 以下`sample_data.py` 中的代码实现了我们的目标：
...
**聚类** 是一种  机器学习算法，旨在基于相似度对项目进行分组。 在此示例中，我们将使用  **道琼斯工业平均指数** （ **DJI** 或 **DJIA** ）  索引进行聚类。 本食谱的大多数步骤已通过前面各章的审查。
...
*   相关文档  位于[，http：//scikit-learn.org/stable/modules/generation/sklearn.cluster.AffinityPropagation.html](http://scikit-learn.org/stable/modules/generated/sklearn.cluster.AffinityPropagation.html)
...
可以从 [http://statsmodels.sourceforge.net/install.html](http://statsmodels.sourceforge.net/install.html) 下载 Source 和  二进制文件。 如果要从源代码安装，请运行以下命令：
...
`statsmodels`程序包具有许多统计测试。 我们将看到这样一个测试的示例-  **Anderson-Darling 测试** 用于  正常性（ [http：// en .wikipedia.org / wiki / Anderson％E2％80％93Darling_test](http://en.wikipedia.org/wiki/Anderson%E2%80%93Darling_test) ）。

## 如何执行...

我们将像以前的配方一样  下载价格数据，但这一次是单只股票。 再次，我们将计算该股票收盘价的对数收益，并将其用作正态性检验函数的输入。
...
如`statsmodels`中所示，此配方证明了安德森-达林（Anderson-Darling）统计检验的正态性。 我们使用没有正态分布的股票价格数据作为输入。 对于  数据，我们获得了`0.13`的 p 值。 由于  的概率在 0 到 1 之间，这证实了我们的假设。
...
**角点检测** （ [http://en.wikipedia.org/wiki/Corner_detection](http://en.wikipedia.org/wiki/Corner_detection) ）是计算机视觉中的标准  技术。 scikit-image 提供了 **哈里斯角** **检测器** ，它很棒，因为角检测非常复杂。 显然，我们可以从头开始做，但是这违反了不重新发明轮子的基本原则。
...
*   相关  scikit-image 文档位于 [http://scikit-image.org/docs/dev/auto_examples/plot_corner.html](http://scikit-image.org/docs/dev/auto_examples/plot_corner.html)

# 检测边缘

**边缘检测** 是  另一种流行的图像处理技术（ [http://en.wikipedia.org/wiki/Edge_detection](http://en.wikipedia.org/wiki/Edge_detection) ）。 scikit-image 具有基于高斯分布的标准偏差的 **Canny 滤镜** 实施  ，可以开箱即用地执行边缘检测。 除了将图像数据作为 2D 数组外，此滤镜还接受以下参数：
...
我们将使用与先前配方相同的  图像。 代码几乎相同（请参见`edge_detection.py`）。 请特别注意我们称为 Canny 筛选器函数的行：
...
*   相关  文档位于 [http://scikit-image.org/docs/dev/auto_examples/plot_canny.html](http://scikit-image.org/docs/dev/auto_examples/plot_canny.html)

# 安装熊猫

**大熊猫** 是用于数据分析的  Python 库。 它与 R 编程语言有一些相似之处，并非偶然。 R 是一种受数据科学家欢迎的专业编程语言。 例如，R 启发了熊猫的核心`DataFrame`对象。
...
*   相关  文档位于 [http://pandas.pydata.org/pandas-docs/stable/install.html](http://pandas.pydata.org/pandas-docs/stable/install.html)

# 估计股票收益与熊猫的相关性

大熊猫  `DataFrame`是矩阵  和类似字典的数据结构，类似于 R 中提供的功能。实际上，它是熊猫的中心数据结构，您可以应用 各种操作就可以了。 例如，查看投资组合的相关矩阵是很常见的，所以让我们开始吧。

## 如何执行...

首先，我们将为每个符号的每日日志返回创建带有熊猫的`DataFrame`。 然后，我们将在约会中加入这些。 最后，将打印相关性，并显示一个图：
...
*   与  相关的文档  位于 [http://pandas.pydata.org/pandas-docs/dev/generation/pandas.DataFrame.html](http://pandas.pydata.org/pandas-docs/dev/generated/pandas.DataFrame.html)
*   *第 4 章* ， *大熊猫入门书* ，摘自 Ivan Idris 的  书 *Python 数据* *分析* ， *Packt Publishing*

# 从 statsmodels 中将数据作为 pandas 对象加载

statsmodels 在其分布中有很多样本  数据集。 完整列表可以在 [https://github.com/statsmodels/statsmodels/tree/master/statsmodels/datasets](https://github.com/statsmodels/statsmodels/tree/master/statsmodels/datasets) 中找到。
...
在本节中，我们将从 statsmodels 中加载数据集作为熊猫`DataFrame`或`Series`对象。
...
*   相关  文档位于 [http://statsmodels.sourceforge.net/stable/datasets/index.html](http://statsmodels.sourceforge.net/stable/datasets/index.html)

# 重采样时间序列数据

在此  教程中，您将学习如何使用熊猫对时间序列进行重新采样。

## 如何执行...

我们将下载 AAPL 的每日价格时间序列数据，然后通过计算平均值将其重新采样为每月数据。 我们将通过创建熊猫`DataFrame`并调用其`resample()` 方法来做到这一点：

1.  在创建大熊猫`DataFrame`之前，我们需要创建一个`DatetimeIndex`对象传递给`DataFrame`构造函数。 根据下载的报价数据创建索引，如下所示：
...
我们根据日期和时间列表创建了日期时间索引。 然后，该索引用于创建熊猫`DataFrame`。 然后，我们对时间序列数据进行了重新采样。 单个字符给出重采样  频率，如下所示：
...
*   相关  大熊猫文档位于[，http：//pandas.pydata.org/pandas-docs/dev/genic/pandas.DataFrame.resample.html](http://pandas.pydata.org/pandas-docs/dev/generated/pandas.DataFrame.resample.html)