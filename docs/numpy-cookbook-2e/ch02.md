...
NumPy 以  其高效的数组而闻名。 之所以成名，部分原因是索引容易。 我们将演示使用图像的高级索引技巧。 在深入研究索引之前，我们将安装必要的软件-SciPy 和 PIL。 如果您认为有此需要，请参阅第 1 章， *和 IPython 一起缠绕 *安装 matplotlib* 秘籍* 。
...
在第 1 章， *和 IPython* 一起缠绕中，我们讨论了如何安装`setuptools`和`pip`。 如有必要，请重新阅读秘籍。
...
*   *在第 1 章和 *中安装 matplotlib* 以及 IPython*
*   *在本章中安装 SciPy*
*   *在本章中安装 PIL*
