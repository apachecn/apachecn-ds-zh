...
如有必要，请安装 PIL 和 SciPy。 有关说明，查阅本秘籍的“另请参阅”部分。
...
1.  在前面的章节中，我们看到了如何加载 LenaSöderberg 的样例图像。 创建一个填充零的数组，并使用图像数据填充 alpha 通道：
...
数组  接口是用于与其他 Python 应用通信的另一种机制。 顾名思义，该协议仅适用于类似数组的对象。 进行了示范。 让我们再次使用 PIL，但不保存文件。
...
MATLAB 及其开放源代码 Octave 是  流行的数学应用。 `scipy.io`程序包具有`savemat()`函数，该功能允许您将 NumPy 数组存储为`.mat`文件作为 Python 字典的值。
