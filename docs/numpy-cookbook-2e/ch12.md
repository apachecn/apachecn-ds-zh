...
在此秘籍中，我们  将查看由 24 小时值计算出的每日平均海平面压力（0.1 hPa）。 这包括打印描述性统计数据和可视化概率分布。 在自然界中，我们经常处理正态分布，因此使用第 10 章，*和 Scikits* 进行正态性检验会派上用场。
...
如果尚未安装  ，请安装`statsmodels`以进行正常性测试（请参阅*安装 scikits-statsmodels* 秘籍第 10 章，*与 Scikits* 一起玩）。
...
*   *使用 Statsmodels* 秘籍执行正常性测试，该秘籍来自第 10 章， *Scikits*
...
如有必要，请按照第 2 章，*高级索引的*安装 SciPy* 秘籍中的说明安装  SciPy。 和阵列概念*。
