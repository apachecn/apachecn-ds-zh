...
Pandas 中有三种主要的数据结构：系列，数据帧架和面板。 与 Numpy `ndarrays`相比，pandas 数据结构更易于使用且更加用户友好，因为在 DataFrame 和 Panel 的情况下，它们提供行索引和列索引。 DataFrame 对象是 Pandas 中最流行和使用最广泛的对象。 在下一章中，我们将讨论 Pandas 索引的主题。