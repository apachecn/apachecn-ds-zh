...
在本章中，我们将重点关注 list 和数据帧，它们具有与序列和数据帧等效的 Pandas。
...
*   `[[`：用于提取 list 或数据帧的元素； 并且只能用于提取单个元素：返回的元素的类型不一定是列表或数据帧。*   `$`：用于按名称提取列表或数据帧的元素，类似于`[[`。
...
在 R DataFrames 和 Pandas 数据帧s 中选择数据遵循类似的脚本。 以下部分说明了我们如何从这两种方法中执行多列选择。
