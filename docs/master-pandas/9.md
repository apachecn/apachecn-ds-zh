...
在本章中，我们研究了可供熊猫用户使用的各种库。 本章旨在作为简短指南，帮助用户围绕熊猫提供的各种模块和库进行导航和查找。 它提供了有关库代码组织方式的分解，还提供了有关各个模块的简要说明。 对于感兴趣的用户来说，这对他们下面的熊猫的内部工作以及希望为代码库做出贡献的用户来说，将是最有价值的。 我们还将简要演示如何使用 Python 扩展来提高性能。 将讨论的各种主题如下：
...
通常，在安装时，会将熊猫作为 Python 模块安装在第三方 Python 模块的标准位置：
...
| 视窗 | `prefix\Lib\site-packages` | `C:\Python27\Lib\site-packages` |
...
*   `frame.py`：这定义了熊猫的 DataFrame 类及其各种方法。 DataFrame 继承自 NDFrame。 （见下文）。
*   `generic.py`：这定义了通用 NDFrame 基类，它是熊猫的 DataFrame，Series 和 Panel 类的基类。 NDFrame 源自`base.py`中定义的 PandasObject。 NDFrame 可以看作是熊猫 DataFrame 的 N 维版本。 有关此的更多信息，请访问 [http://nullege.com/codes/search/pandas.core.generic.NDFrame](http://nullege.com/codes/search/pandas.core.generic.NDFrame) 。
...
*   `index.py`：这定义了 Index 类及其相关功能。 所有熊猫的对象（系列，数据框和面板）都使用索引来存储轴标签。 它的下面是一个不可变的数组，提供了可以切片的有序集合。
*   `internals.py`：这定义了多个对象类。 这些列出如下：
    *   `Block`：这是具有熊猫其他功能的同质 N 维`numpy.ndarray`对象。 例如，它使用`__slots__`将对象的属性限制为“ ndim”，“ values”和“ _mgr_locs”。 它充当其他`Block`子类的基类。
...
*   `ops.py`：这定义了熊猫对象的算术运算。 它不是公共 API。
*   `panel.py`，`panel4d.py`和`panelnd.py`：这些提供了熊猫的 Panel 对象的功能。
...
*   `json.py`：这是用于处理熊猫中 json I / O 的模块。 这定义了`Writer`，`SeriesWriter`，`FrameWriter`，`Parser`，`SeriesParser`和`FrameParser`类。
*   `packer.py`：这是 msgpack 序列化程序支持，用于将熊猫数据结构读取和写入磁盘。
*   `parsers.py`：此模块定义了各种功能和类，这些功能和类用于解析和处理文件以创建熊猫的 DataFrame。 下文讨论的所有三个`read_*`功能都具有多个可配置的读取选项。 有关更多详细信息，请参见此参考： [http://bit.ly/1e4Xqo1](http://bit.ly/1e4Xqo1) 。
...
*   `stata.py`：此工具包含用于将`Stata`文件处理为熊猫 DataFrame 的工具。
...
*   `rplot.py`：这是一个模块，提供在熊猫中生成网格图的功能。
...
*   `testing.py`：这提供了断言，调试，单元测试以及其他用于测试的类/函数。 它包含许多特殊的 assert 函数，这些函数使检查 Series，DataFrame 或 Panel 对象是否等效的操作更加容易。 其中一些功能包括`assert_equal(..)`，`assert_series_equal(..)`，`assert_frame_equal(..)`和`assert_panelnd_equal(..)`。 `pandas.util.testing`模块对熊猫代码库的贡献者特别有用。 它定义了一个`util.TestCase`类。 它还提供了用于处理语言环境，控制台调试，文件清除，比较器等的实用程序，以供潜在的代码库贡献者进行测试。
...
*   `clipboard.py`：这包含跨平台剪贴板方法，可通过键盘启用复制和粘贴功能。 熊猫 I / O API 包括`pandas.read_clipboard()`和`pandas.to_clipboard(..)`之类的功能。
...
*   `common.py`：这提供了许多功能，可以将熊猫对象转换为等效的 R 版本
*   `mass.py`：这是`rlm` -R 的`lm`函数的未实现版本
*   `var.py`：其中包含未实现的类`VAR`

## 熊猫/测试

这个模块为熊猫中的各种物体提供了许多测试。 特定库文件的名称是不言自明的，除了邀请读者进行探索之外，在这里我将不进一步详细介绍。
...
*   `pickle_compat.py`：此功能提供了在 0.12 之前的版本中腌制熊猫对象的功能。
...
*   `period.py`：这为熊猫`TimeSeries`定义了`Period`和`PeriodIndex`类。
...
该模块处理将熊猫 DataFrame 集成到 PyQt 框架中。 有关 PyQt 的更多信息，请访问
...
*   标题为*，[的*提高性能的熊猫文档 http://pandas.pydata.org/pandas-docs/stable/enhancingperf.html*](http://pandas.pydata.org/pandas-docs/stable/enhancingperf.html)*
...
总结本章，我们浏览了熊猫的图书馆层次结构，试图说明图书馆的内部运作方式。 我们还谈到了使用 Python 扩展模块来提高代码性能的好处。