...
*   `generic.py`：这定义了通用 NDFrame 基类，它是 Pandas 的 DataFrame，Series 和 Panel 类的基类。 NDFrame 源自`base.py`中定义的 PandasObject。 NDFrame 可以看作是 Pandas DataFrame 的 N 维版本。 有关此的更多信息，请访问[这里](http://nullege.com/codes/search/pandas.core.generic.NDFrame)。
...
*   `parsers.py`：此模块定义了各种功能和类，这些功能和类用于解析和处理文件以创建 Pandas 的 DataFrame。 下文讨论的所有三个`read_*`功能都具有多个可配置的读取选项。 有关更多详细信息，[请参见此参考](http://bit.ly/1e4Xqo1)。
...
*   `sql.py`：它是类和功能的集合，用于使能够从试图与数据库无关的关系数据库中检索数据。 讨论如下：
    ...
    *   `read_sql_table(..)`：这将 SQL db 表读入数据帧。    *   `read_sql_query(..)`：这会将 SQL 查询读取到 DataFrame 中。
    *   `read_sql(..)`：这将 SQL 查询/表读入数据帧。*   `to_sql(..)`：此操作将 DataFrame 中存储的记录写入 SQL 数据库。
