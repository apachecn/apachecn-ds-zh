...
`groupby`子句是对 DataFrame 的操作。 系列是一维对象，因此对其执行`groupby`操作不是很有用。 但是，它可用于获取系列的不同行。 `groupby`操作的结果不是 DataFrame，而是 DataFrame 对象的`dict`。 让我们从涉及世界上最受欢迎的运动-足球的数据集开始。

该数据集来自 Wikipedia，其中包含自 1955 年成立以来欧洲俱乐部冠军杯决赛的数据。有关参考，您可以访问 [http://en.wikipedia.org/wiki/UEFA_Champions_League](http://en.wikipedia.org/wiki/UEFA_Champions_League) 。

使用以下命令将`.csv`文件转换为 DataFrame：
...
因此，输出显示了赛季，获胜和亚军俱乐部所属的国家，得分，场地和出勤人数。 假设我们要按获得的欧洲俱乐部冠军的数量来对各国进行排名。 我们可以使用`groupby`来做到这一点。 首先，我们将`groupby`应用于 DataFrame 并查看结果的类型是什么：
...
此信息的来源位于 [http://soccerstats.com](http://soccerstats.com) 。

现在让我们像往常一样将目标统计数据读入 DataFrame 中。 在这种情况下，我们使用月份在 DataFrame 上创建一个行索引：
...
然后，我们可以遍历生成的`groupby`对象并显示组。 在以下命令中，我们看到按年份分组的两组统计信息。 请注意，使用 lambda 函数从月份的第一天开始获取年份组。 有关 lambda 函数的更多信息，请转到 [http://bit.ly/1apJNwS](http://bit.ly/1apJNwS) ：
...
注意，由于在前面的命令中我们将索引分组，因此需要指定级别参数，而不是仅使用列名。 当我们按多个键分组时，得到的分组名称是一个元组，如后面的命令所示。 首先，我们重置索引以获得原始 DataFrame 并定义一个 MultiIndex 以便能够按多个键进行分组。 如果不这样做，将导致`ValueError`：
...
如果我们的 DataFrame 具有 MultiIndex，则可以使用`groupby`按层次结构的不同级别分组并计算一些有趣的统计数据。 这是使用由`Month`和`Stat`组成的 MultiIndex 的目标统计数据：
...
1.  获得每个游戏数据的目标作为 DataFrame。 请注意，由于`gpg`作为系列返回，因此我们必须对其进行转置：
...
下图显示了我们讨论过的 1955-2012 年欧洲联赛每场比赛的进球数。 可以在 [http://mattstil.es/images/europe-football.png](http://mattstil.es/images/europe-football.png) 中找到其来源。
...
这将生成一个分组的 DataFrame 对象，该对象在前面的命令中显示。 我们还将浮点格式重置为`None`，因此由于上一节中的格式设置，整数值数据将不会显示为浮点。

### 应用多种功能

对于分组的 DataFrame 对象，我们可以指定要应用于每列的函数列表：
...
对于分组的系列，我们可以传递函数列表或`dict`。 在前面的情况下，指定了`dict`，并且将键值用作结果 DataFrame 中列的名称。 请注意，在单个样本大小的组的情况下，标准偏差未定义，结果为`NaN`，例如，罗马尼亚。

### transform（）方法

`groupby-transform`功能用于对`groupby`对象执行转换操作。 例如，我们可以使用`fillna`方法替换`groupby`对象中的`NaN`值。 使用转换后得到的对象具有与原始`groupby`对象相同的大小。 让我们考虑一个数据框架，该数据框架显示四个足球联赛中每个月的得分目标：
...
`concat`函数用于沿指定的轴连接多个熊猫的数据结构，并可能沿其他轴执行合并或相交操作。 以下命令说明`concat`功能：
...
*   `objs`功能：要串联的 Series，DataFrame 或 Panel 对象的列表或字典。
...
有关其余选项的说明，请参阅 [http://pandas.pydata.org/pandas-docs/stable/merging.html](http://pandas.pydata.org/pandas-docs/stable/merging.html) 上的文档。
...
第三种情况使我们能够使用原始 DataFrame 中的特定索引进行联接：
...
为了保持类似于原始 DataFrame 的列顺序，我们可以应用`reindex_axis`函数：
...
请注意，对于前两行，后两列的值为`NaN`，因为第一个 DataFrame 仅包含前三列。 `append`函数无法在某些地方工作，但是会返回一个新的 DataFrame，并将第二个 DataFrame 附加到第一个 DataFrame 上。

## 将一行附加到 DataFrame

我们可以通过将一系列或字典传递给`append`方法来将单个行附加到 DataFrame：
...
## 类似于 SQL 的 DataFrame 对象的合并/联接

`merge`函数用于获取两个 DataFrame 对象的联接，类似于 SQL 数据库查询中使用的那些联接。 DataFrame 对象类似于 SQL 表。 以下命令对此进行了说明：
...
可以在 [http://pandas.pydata.org/pandas-docs/stable/merging.html](http://pandas.pydata.org/pandas-docs/stable/merging.html) 中找到上述信息的来源。
...
可在 [http://finance.yahoo.com](http://finance.yahoo.com) 上找到此信息的来源。
...
指定`outer`会使用两个 DataFrame 中的所有键（联合），这将提供在先前输出中指定的三行。 由于并非所有列都存在于两个 DataFrame 中，因此对于不属于交集的 DataFrame 中的每一行，来自另一个 DataFrame 的列均为`NaN`。
...
在这里，我们看到左侧 DataFrame `slice3`的键用于输出。 对于`slice3`中不可用的列，即`Russell 2000`和`DJIA, NaN`用于`TradingDate`为 2014/02/03 的行。 这等效于 SQL 左外部联接。
...
这等效于 SQL 右外部联接。 有关 SQL 连接如何工作的简单说明，请参考 [http://bit.ly/1yqR9vw](http://bit.ly/1yqR9vw) 。

### 联接功能

`DataFrame.join`函数用于合并两个具有不同列且没有共同点的数据框。 本质上，这是两个数据帧的纵向联接。 这是一个例子：
...
在这种情况下，我们看到结果是来自两个数据框的列的组合。 让我们看看当尝试将 join 与两个具有共同列的 DataFrame 一起使用时会发生什么：
...
由于列重叠而导致异常。 您可以在[的官方文档页面中找到有关使用`merge`，`concat`和`join`操作的更多信息，http://pandas.pydata.org/pandas-docs/stable/merging.html](http://pandas.pydata.org/pandas-docs/stable/merging.html) 。
...
在这里，使用与组的不同值相对应的列创建数据框架，或者用统计学的话来说，就是因子水平。 通过熊猫`pivot_table`函数可以达到相同的结果，如下所示：
...
`pivot_table`的完整提要可从 [http://bit.ly/1QomJ5A](http://bit.ly/1QomJ5A) 获得。 您可以在以下位置找到有关其用法的更多信息和示例： [http://bit.ly/1BYGsNn](http://bit.ly/1BYGsNn) 和 [https://www.youtube.com/watch?v=mCLuwCql5t4](https://www.youtube.com/watch?v=mCLuwCql5t4) 。

## 堆放

除支点功能外，`stack`和`unstack`功能在 Series 和 DataFrames 上也可用，它们可用于包含 MultiIndexes 的对象。
...
在这里，我们看到行索引由组上的 MultiIndex 和以 weight 列作为数据值的观察组成。 现在，让我们看看如果将`unstack`应用于`group`级别会发生什么：
...
在这里，我们可以看到 DataFrame 已旋转，并且该组现在已从行索引（标题）更改为列索引（标题），从而使 DataFrame 看起来更加紧凑。 为了更详细地了解正在发生的事情，我们首先将 MultiIndex 作为行索引放在组的观察上：
...
在这里，我们看到我们得到的不是原始的堆栈 DataFrame，因为堆栈级别-即`'group'—`成为列 MultiIndex 中新的最低级别。 在原始堆叠的 DataFrame 中，`group`是最高级别。 这是对`stack`和`unstack`的完全可逆的调用序列。 默认情况下，`unstack()`函数会取消堆叠最后一个级别，即`observation`，如下所示：
...
还有许多其他与重塑 DataFrame 有关的方法。 我们将在这里讨论。

### 使用融化功能

`melt`函数使我们能够通过将 DataFrame 的某些列指定为 ID 列来转换它。 这样可以确保在进行任何重要的转换后，它们始终保持为列。 其余的非 ID 列可被视为变量，并可进行透视设置并成为名称-值两列方案的一部分。 ID 列唯一标识 DataFrame 中的一行。
...
此函数用于将分类变量转换为指标 DataFrame，该指标本质上是分类变量可能值的真值表。 下面的命令是一个示例：
...
可以在 [http://vincentarelbundock.github.io/Rdatasets/csv/datasets/PlantGrowth.csv](http://vincentarelbundock.github.io/Rdatasets/csv/datasets/PlantGrowth.csv) 中找到上述数据的来源。

# 摘要

在本章中，我们看到了各种方法来重新排列熊猫中的数据。 我们可以使用`pandas.groupby`运算符和`groupby`对象上的关联方法对数据进行分组。 我们可以使用`concat`，`append`，`merge`和`join`函数合并和合并`Series`和`DataFrame`对象。 最后，我们可以使用`stack` / `unstack`和`pivot` / `pivot_table`函数重塑和创建`pivot`表。 这对于显示数据以进行可视化或准备数据以输入其他程序或算法非常有用。

在下一章中，我们将研究一些数据分析中有用的任务，可以应用熊猫，例如处理时间序列数据以及如何处理数据中的缺失值。

要获得有关这些主题的更多信息，请访问 [http://pandas.pydata.org/pandas-docs/stable/](http://pandas.pydata.org/pandas-docs/stable/) 上的官方文档。