...
在每种情况下都会返回一个布尔型数据帧。 在`datetime`和 Pandas 时间戳记中，缺失值由`NaT`值表示。 对于基于时间的类型，这相当于 Pandas 中的`NaN`。
...
3.  We can also interpolate and fill in the missing values by using the `interpolate()` function, as explained in the following commands:
    ...
    `interpolate()`函数还采用一个参数-表示该方法的*方法*。 这些方法包括线性，二次，三次样条等等。 您可以从[官方文档](http://pandas.pydata.org/pandas-docs/stable/missing_data.html#interpolation)中获取更多信息。
...
那么，为什么“时间戳记”列看起来有点奇怪？ 好吧，滴答数据时间戳通常以纪元时间表示（有关更多信息，请参考[这里](http://en.wikipedia.org/wiki/Unix_epoch)），作为一种更紧凑的存储方式。 我们需要将其转换为更易于理解的时间，我们可以按照以下步骤进行操作：
...
这些偏移量可用作`date_range`和`bdate_range`函数的参数，以及用作`PeriodIndex`和`DatetimeIndex`等索引类型的构造函数。 可以在 [Pandas 文档](http://pandas.pydata.org/pandas-docs/stable/timeseries.html#) 中找到对此的全面讨论。
...
这些数据类型均具有与之关联的索引数据类型：`Timestamp` / `Datetime`的`DatetimeIndex`和`Period`的`PeriodIndex`。 这些索引数据类型基本上是`numpy.ndarray`的子类型，包含对应的 Timestamp 和 Period 数据类型，并且可用作序列和数据帧对象的索引。
...
Series 和数据帧有一个 plot 方法，它只是`plt.plot`的包装。 在这里，我们将研究如何绘制正弦和余弦函数的简单图。 假设我们希望在 pi 到 pi 间隔上绘制以下函数：
