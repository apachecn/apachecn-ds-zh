...
我们将绘制三个具有不同选项的地块。 对于第一个，我们将使用`hue`变量，对于，将第二个使用`col`变量以及参数回归模型， 对于第三个变量，我们将  col 变量与非参数回归模型一起使用。
...
*   `sns.heatmap(sales, annot=True, fmt='.0f',linewidths=.5, robust=True, vmin=200, vmax=600, cmap="YlGnBu", yticklabels=False, ax=ax3)`绘制了另一个热图，并在色条上设置了限制：
    *   `vmin=200`是的下限，`vmax=600`是上限。 低于，下限的数据中的所有数字都以，色标的下限极端和  的数字高于且的上限为彩色 极限将使用上极限色上色。
