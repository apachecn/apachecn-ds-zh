...
*   `g = sns.clustermap(wine_quality.drop('Quality',axis=1), standard_scale=1, robust=True, row_colors=row_colors, cmap='viridis')`绘制集群图：
    ...
    *   我们可以尝试使用，`method`参数分配链接的不同方法，以及使用`metric`参数分配距离的指标。 有关更多详细信息，请参考 [https://docs.scipy.org/doc/scipy/reference/genic/scipy.spatial.distance.pdist.html](https://docs.scipy.org/doc/scipy/reference/generated/scipy.spatial.distance.pdist.html) 。
