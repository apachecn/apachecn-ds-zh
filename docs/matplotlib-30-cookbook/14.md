...
*   `def marginal_boxplot(a, vertical=False, **kws):`是在  边沿轴域上绘制  箱形图的功能：
...
6.  在第二个图形对象上定义，并在单独的轴域上绘制  热图和色条，并使用水平色条：
...
*   `grid_kws = {"height_ratios": (.9, .05), "hspace": .4}`是网格的参数字典。 和两个轴的高度之比为`0.9:0.05`，因此和色条轴域的高度与绘图高度相比非常短。 两个轴之间的水平间距为 0.4。
