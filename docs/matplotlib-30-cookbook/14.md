...
`jointplot()`和`JointGrid()`启用，将创建三个轴/图作为一个图形。 主轴域称为**联合图**，其他两个称为**边缘轴**。 边缘轴之一在联合图的顶部，，第二边缘轴在联合图的右侧。 在联合图上绘制了  两个变量之间的关系，并且在每个边缘轴上绘制了这两个变量中的每一个的单变量分布。 这些函数具有各种参数，可以灵活地在这三个轴的每一个上选择和类型的图形。
...
*   `for quarter, sales in snacks_sales.groupby('Quarter'):`是用于按季度对销售数据进行分组的`for`循环，因此它们可以在边缘轴上绘制，而双变量数据可以在  联合轴上绘制：
    *   `sns.kdeplot(sales["Cookies"], ax=g4.ax_marg_x, label='Q'+str(i))`在顶部边缘轴上使用标签 Q1，Q2，Q3 和 Q4 绘制 KDE 图，在`for`循环的每个迭代中使用一个。
...
*   对于第二个绘图，`mask = np.zeros_like(corr)`使用`corr`矩阵的相同尺寸创建遮罩，并用零填充。
