...
*   `x, y, z = iris['petal_length'], iris['petal_width'], iris['species']`为  *x*，*y* 和 *z* 坐标准备数据。 我们在 *x* 和 *y* 轴上绘制`*petal_length*`和`petal_width`，然后在 *z* 轴上绘制`species`（簇号）。
...
*   `ax.bar(X[i:i+5], Y[:,int(i/5)], zs=k, zdir='y', color=cs, alpha=0.8)`绘制，条形图：
    ...
    *   `zdir='y'`指定交换 *z* 和 *y* 轴，以便在 *y* 轴绘制`Z`数据。 在 *z* 轴上绘制`y`数据。
...
*   `ax.bar(x, y, zs=0, zdir='y', label='Number of Defects in (X,Z)')`绘制条形图：
    ...
    *   `zdir='y'`交换 *y* 和 *z* 轴，以便在 *z* 轴上绘制`y`数据。 在 *y* 轴上绘制`z`数据，在这种情况下为零。
    *   指定的标签将显示在图例中。
