...
在第 4 章和“开发可视化来提高发布质量”中，我们学习了如何使用双生轴域方法在图的左右轴域上绘制两个不同的比例尺。 在第 9 章和“开发交互式绘图”中，我们学习了如何在 *y* 轴域上绘制不同的测量单位，并再次使用`twinx`方法进行交互式分析 。 在本食谱中，我们将学习如何使用 Matplotlib `twinx`主方法以及`axisartist`和`axes_grid1`工具包在 *y* 轴域上绘制多个比例尺。
