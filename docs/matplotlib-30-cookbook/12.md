...
在本章中，我们将学习`axes_grid1` 工具包的功能。 我们将介绍以下食谱：

*   使用`axisartst`和`axesgrid1`工具包绘制双轴
*   使用 AxesDivider 绘制散点图和相关的直方图
*   使用 AxesDivider 绘制颜色条
*   使用`ImageGrid`在网格中用颜色条绘制图像
*   使用`inset_locator`放大图像
*   使用`inset_locator`绘制插入轴

# 介绍

`axes_grid1`工具包旨在提供一组帮助器类，以在网格中绘制带有颜色条的图像，并根据图像大小正确对齐颜色条。 它可用于放大图像的一部分，绘制插入轴（类似于图片中的图片），绘制二维图和两个变量，寄生轴和锚定艺术家中每个变量的关联直方图。

# 使用 axisartist 和 axesgrid1 工具箱绘制双轴

在[第 4 章](../Text/4.xhtml)和*开发可视化以提高发布质量*中，我们学习了如何使用双轴方法在图的左右轴上绘制两个不同的比例尺。 在[第 9 章](../Text/8.xhtml)和*开发交互式图*中，我们学习了如何在 *y* 轴上绘制不同的测量单位，并再次使用`twinx`方法进行交互式分析 。 在本食谱中，我们将学习如何使用 Matplotlib `twinx`主方法以及`axisartist`和`axes_grid1`工具包在 *y* 轴上绘制多个比例尺。

# 做好准备

我们将首先使用 Matplotlib `twinx()`方法绘制图形。 然后，在*的更多内容...* 部分中，我们将使用`axisartist`和`axes_grid1`工具包再次绘制图形。
...
1.  定义图形和主轴（左侧为`ax1`， *y* 轴），如下所示：

```py
fig, ax1 = plt.subplots(1,1)
```

2.  定义双轴（右侧为`ax2`， *y* 轴，右侧为`ax3`，另一 *y* 轴）：
...
3.  放置`ax3`使其不与`ax2`右轴重叠：

```py
ax3.spines["right"].set_position(("axes", 1.15))
```

4.  准备数据并在主轴`ax1`上绘制产品缺陷条形图：
...
5.  在右轴`ax2`上的折线图中准备数据并按百分比绘制累积缺陷：
...
6.  准备数据并在第二个右轴`ax3`上绘制累积的缺陷计数折线图：
...
7.  定义步长为`2`的主定位器，并将其应用于主轴`ax1`的 *y* 轴：
...
*   `ax2 = ax1.twinx()`和`ax3 = ax1.twinx()`创建名为`ax2`和`ax3`的逻辑轴，它们共享主轴（`ax1` *）*的 *x* 轴。 *y* 轴在右侧。
*   `ax3.spines["right"].set_position(("axes", 1.15))`将第二个 *y* 轴向右移动主轴宽度的 15％。 如果我们不这样做，则`ax2`和`ax3` *y* 轴将相互重叠，并在同一轴上同时显示它们的刻度和刻度标签。
...
*   `majorLocator = MultipleLocator(2)`以`2`的步长定义定位器，`ax1.yaxis.set_major_locator(majorLocator)`将定位器应用于主轴`ax1`的 *y* 轴。
*   `ax3.set_ylabel('Cumulative Defects Count', labelpad=10)`将 *y* 轴标签设置为最右边的轴，而`labelpad=10`指定该轴与其标签之间的间隙，以使标签不与该轴的刻度标签重叠。
*   `ax1.legend(graphs, [g.get_label() for g in graphs], loc=5)`绘制图例如下：
    *   `[g.get_label() for g in graphs]`获取在绘制图形时设置的每个图形的标签列表。
    *   `loc=5`指定将图例绘制在轴的右侧。 有关放置图例的更多选项，请参考[第 4 章](../Text/4.xhtml)和*开发可视化以提高发布质量*。

*   `ax1.yaxis.label.set_color(b.patches[0].get_facecolor())`设置主轴的 *y* 轴标签的颜色，如下所示：
...
*   `ax2.yaxis.label.set_color(l.get_color())` 和 `ax3.yaxis.label.set_color(l1.get_color())` 获得线图 `l` 和 `l1` 的颜色，并将它们应用于各自的标签。
*   `ax2.spines["right"].set_edgecolor(l.get_color())`设置右轴的颜色，用于绘制线图 `l`。
...
*   而不是使用`plt.subplots()`，这里我们使用`ax1 = host_subplot(111, axes_class=AA.Axes)`，它是`axes_grid1`工具包的辅助函数，它以`axisartist`作为参数。 这意味着`ax1` 轴获得`host_subplot()`和`axisartist`的属性。
...
*   我们为标签，刻度和刻度标签设置各种属性的方式与我们使用 axisartist 工具包在[第 12 章](../Text/11.xhtml)和*中了解的方法相同。*
*   但是，某些属性无法与原始轴类一起正常工作，因为`axisartist`不能继承原始轴类的所有属性。
*   当您使用交互式后端打开该图并放大图形的某些部分时，只有 *y* 主轴限制发生变化，而其他 *y* 轴的数据则发生变化。 右（寄生轴）与初始设置相同。 如果不使用`axes_grid1`和`axisartist`来绘制先前的图形，情况并非如此，其中所有 *y* 轴限制都会更改以反映图形的当前部分。
...
在本食谱中，我们将学习如何使用`axes_grdi1`的 AxesDivider 类在主轴上绘制双变量图，并在主轴的任意两侧绘制两个单变量图。 这有助于可视化两个变量之间的关系，以及同一图中两个变量的分布情况（尽管三个不同的轴/曲线）。

从技术上讲，在主轴上绘制的变量和在主轴两侧上的单变量图可以不同。 您可以从主轴的四个侧面中选择任意两个侧面以进行单变量图绘制。 但是，通常的做法是绘制在主轴的顶部和右侧。

在此配方中，我们将在带有两个变量的主轴上绘制散点图，并且在主轴的顶部和右侧，将为散点图所使用的两个变量分别绘制直方图。
...
4.  定义轴分割器，以使一个轴位于主轴的顶部，另一个轴位于主轴的右侧，如下所示：
...
5.  使刻度标签对顶部轴的 *x* 轴以及右轴的 *y* 轴不可见，以使它们不与 主轴：
...
7.  将顶部轴设置为`yticks`，将右侧轴设置为`xticks`：
...
*   前面的示例已经对您熟悉了前三个步骤。
*   `divider = make_axes_locatable(axmain)`定义轴分频器。 它实质上是定位给定轴的边界。
*   `axtop = divider.append_axes("top", size=1.5, pad="15%", sharex=axmain)`在主轴上方定义一个新轴，如下所示：
    *   `"top"`指定要在当前轴之上绘制的新轴。
    *   `size=1.5`指定此新轴的高度为 1.5 英寸，其宽度与主访问宽度相同。
    *   `pad=15%`指定主轴与顶部新轴之间的间隙为主轴高度的 15％。
    *   `sharex=axmain`指定此新轴应与主轴共享 *x* 轴属性，例如轴标签，刻度和刻度标签
*   `axright = divider.append_axes("right", "100%", 0.1, sharey=axmain)`在主轴右侧定义了一个新轴，如下所示：
    *   `"right"`指定新轴应绘制在主轴的右侧。
    *   `100%`指定新轴的宽度与主轴的宽度相同。 新轴的高度也与主轴的高度相同，因为它们都共享 *y* 轴。
    *   `0.1`是主轴和该新轴之间的填充（以磅为单位）。
    *   `sharey=axmain`指定此新轴应与主轴共享*和*轴属性，例如轴标签，刻度和刻度标签。
*   `axtop.xaxis.set_tick_params(labelbottom=False)`使 *x* 轴标签对于上轴不可见。
*   `axright.yaxis.set_tick_params(labelleft=False)`使 *y* 轴标签对于右轴不可见。
*   `axtop.hist(x, bins=15)`在具有 15 个 bin 的顶轴上绘制直方图。
*   `axright.hist(y, bins=10, orientation='horizontal')`使用水平条形图而不是默认垂直条形图在带有 10 个 bin 的右轴上绘制直方图。
*   其余的语句设置所有三个轴的刻度，标签和标题。
...
在本食谱中，我们将学习如何使用 AxesDivider 绘制图像的颜色条，以确保其与图像正确对齐。 我们已经使用了`pyplot.colorbar()`很多次，但有时色标超出了图像边界。 使用 AxesDivider 和`ImageGrid`，我们将不会看到此问题，如本食谱所示。

我们将在三个轴上绘制图像–一个使用不带 AxesDivider 的常规`pyplot.colorbar()`； 第二个使用 AxesDivider 和右侧的垂直颜色条; 第三个再次使用 AxesDivider，但这一次在图像顶部带有水平色条。
...
4.  使用以下代码，定义第二个子图，绘制图像，定义 AxesDivider，在右侧绘制颜色栏，然后设置 *x* 和 *y* 刻度。
...
*   `plt.subplot(131)`定义 1 x 3 网格上的第一个轴。
*   `plt.imshow(corr_wine, cmap='plasma')`使用  `'plasma'` 颜色图绘制相关图。
...
*   `ax2 = plt.subplot(132)`在 1 x 3 的网格上定义第二个轴。
*   `im1 = ax2.imshow(corr_wine, cmap='Blues')`使用`'Blues'`颜色图绘制相关图。
*   `ax2_divider = make_axes_locatable(ax2)`为第二个轴定义 AxesDivider 函数。
*   `cax1 = ax2_divider.append_axes("right", size="7%", pad=0.03)`使用 AxesDivider 定义了颜色栏的新轴。
*   `"right"`指定将新轴放置在`ax2`的右侧。
*   `size="7"`指定新轴的宽度应为`ax2`宽度的 7％。
*   `pad=0.03`指定`ax2`轴和颜色栏轴之间的间隙应为`ax2`宽度的 3％。
*   `plt.colorbar(im1, cax=cax1)`绘制`ax2`右侧的颜色条，并带有给定的图像和颜色条轴。
...
*   `ax3 = plt.subplot(133)`在 1 x 3 的网格上定义第三条轴。
...
# 使用 ImageGrid 在网格中使用颜色栏绘制图像

在[第 3 章](../Text/3.xhtml)，*绘制多个图形，子图形和图形*和[第 6 章](../Text/5.xhtml)，*绘制高级功能*的过程中，我们学习了如何绘制 网格上具有多个网格选项的多个图。 当您尝试使用带有这些选项的网格上的颜色条来绘制图像时，颜色条对齐可能不是我们想要的方式。 `axes_grid1`工具包的`ImageGrid`辅助功能可在网格中以正确对齐方式绘制带有颜色条的图像。 我们将在本食谱中学习如何使用这些功能。
...
2.  定义尺寸为 12 x 8 英寸的图形：

```py
fig = plt.figure(1, (12, 8.))
```

3.  定义要在第一个轴上绘制的 2 x 2 网格：
...
9.  在先前定义的 2 x 2 网格上绘制具有相同 c 或相关矩阵的四个图像：
...
*   `for i in range(4):` 是用于在四个网格上一一绘制图像的 for 循环：
    *   `im = grid2[i].imshow(images[i], cmap='ocean')` 使用`'ocean'`颜色图绘制图像。
*   `grid2.cbar_axes[0].colorbar(im)`打印颜色条。
*   `grid3 = ImageGrid()` 定义第三个 图像网格，其参数如下：
    *   `fig`指定要绘制此网格的图。
    ...
    *   `cbar_size="10%"`指定彩条宽度应为图像轴宽度的`10%`。
    *   `cbar_pad="3%"`指定彩条和图像之间的距离为图像轴宽度的 3％。
    *   `limits = ((-1, 1), (-1, 0), (0, 1.), (-0.5, 0.5))`指定要应用于四个图像的每个颜色栏的下限和上限，而不是绘制-1 至+1 的整个范围。
*   `for i in range(4):` 是用于在四个网格中一一绘制图像的 for 循环：
    ...
    *   `grid3.cbar_axes[i].colorbar(im)`打印颜色栏。
...
在 [第 6 章](../Text/5.xhtml)和*具有高级功能的绘图* 中，我们学习了如何使用 Matplotlib 的面向对象界面在主轴内绘制插入轴 。 在本食谱和下一篇食谱中，我们将学习如何使用`axes_grid1`工具包的`inset_locator`帮助器功能创建插入轴并绘制图像和其他图形。

在本食谱中，我们将学习如何放大图像的一部分并将其显示在相同主轴的插入轴上。

我们将在两个轴上绘制图像，并在每个图中放大图像的不同部分。
...
7.  设置 *x* 和 *y* 轴的插入轴数据限制，并指定要放大的相关矩阵部分：
...
10.  在选定要缩放的部分周围标记一个边界框，并使用线条将其与缩放的插入轴连接：

```py
mark_inset(ax1, axins1, loc1=2, loc2=4, fc="none", ec="b")
```

11.  对第二个轴重复执行*步骤 4* 至*步骤 8* ，这将显示放大的图像较小的部分：
...
*   `extent = [-2, 2, -3, 1]`指定主轴的 x（-2，3）和 *y* （-3，1）坐标上的数据限制。 有关更多详细信息，请参考[第 6 章](../Text/5.xhtml)，*具有高级功能的绘图*，*起源和范围*中的配方。
*   `ax1.imshow(extended_image, extent=extent, interpolation="nearest", origin="lower")`在主轴`ax1` 上绘制扩展图像。
*   `axins1 = zoomed_inset_axes(ax1, 3, loc=1)`定义插入轴如下：
    *   `ax1`是要在其上创建插入轴的轴。
    *   `3`指定插入轴数据比例是主轴的三倍，因此对于 *x* 和 *y，主轴上的一个单位缩放到插入轴上的三个单位* 轴。
    *   `loc=1`指定将插入轴放置在主轴的右上角。
*   `axins1.imshow(extended_image, extent=extent, interpolation="nearest", origin="lower")`在插入轴上绘制相同的扩展图像，并使用与主轴相同的参数。
*   `axins1.set_xlim(-1.0, -0.5)`和`axins1.set_ylim(-2.0, -1.5)`指定主轴上要放大的图像部分。
*   `axins1.xaxis.get_major_locator().set_params(nbins=8)`和`axins1.yaxis.get_major_locator().set_params(nbins=8)`定义插入轴的 *x* 和 *y* 轴上的刻度数。
...
在本食谱中，我们将学习在主轴上绘制散点图，并在两个插入轴上创建两个变量中每个变量的直方图。 我们将使用熟悉的`Iris`数据集来执行此操作。
...
*   `im = ax.scatter(x, y, c=iris.petal_length, s=100*iris.petal_width, cmap='viridis', alpha=0.7)`在主轴上绘制散点图，如下所示：
    ...
    *   `cmap='viridis'`指定要使用的颜色图。
    *   `alpha=0.7`指定绘图上点的透明度级别。
*   `plt.colorbar(im)`在主轴的右侧绘制颜色栏。
*   `inset_ax1 = inset_axes(ax, width="40%", height=1., loc=4)`定义插入轴 1 的属性，如下所示：
    *   `ax`是要放置插入轴的主轴。
    *   `width="40%"`指定插入轴的宽度应为主轴宽度的`40%`。
    *   `height=1.`指定插入轴的高度应为 1 英寸。
    *   `loc=4`指定将插入轴放置在主轴的右下角。
*   `inset_ax1.hist(iris.sepal_length)`绘制`sepal_length`变量的直方图。
*   `inset_ax1.set_title('sepal legth', size=10, pad=20)`打印尺寸为`10`点的插入轴标题，并将其定位在顶部轴上方`20`点，以使刻度标签和标题不重叠。
...
*   `inset_ax2 = inset_axes(ax, width=1.5, height="25%", loc=2)`定义第二插入轴如下：
    *   `width=1.5`将插入轴的宽度指定为`1.5`英寸。
    *   `height="25%"`指定插入轴的高度为主轴高度的`25%`。
    *   `loc=2`指定将插入轴放置在主轴的左上角。
*   `ax.set(xlabel='sepal length', ylabel='sepal width')`设置主轴的 *x* 和 *y* 轴标签。
