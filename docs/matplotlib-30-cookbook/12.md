...
*   `axtop.hist(x, bins=15)`在具有 15 个桶的顶轴上绘制直方图。
*   `axright.hist(y, bins=10, orientation='horizontal')`使用水平条形图而不是默认垂直条形图在带有 10 个桶的右轴上绘制直方图。
...
*   `extent = [-2, 2, -3, 1]`指定主轴的 x 坐标上的数据限制为`(-2, 3)`和 *y* 为`(-3, 1)`。 有关更多详细信息，请参考第 6 章，“带有高级功能的绘图”，“起源和范围”中的配方。
