...
`axes_grid1`工具包旨在提供一组帮助器类，以在网格中绘制带有颜色条的图像，并根据图像大小正确对齐颜色条。 它可用于放大图像的一部分，绘制插入轴（类似于图片中的图片），绘制二维图和两个变量，从属轴域和锚定艺术家中每个变量的关联直方图。
...
*   `ax2 = ax1.twinx()`和`ax3 = ax1.twinx()`创建名为`ax2`和`ax3`的逻辑轴，它们共享主轴域（`ax1`）的 *x* 轴。 *y* 轴在右侧。
*   `ax3.spines["right"].set_position(("axes", 1.15))`将第二个 *y* 轴向右移动主轴域宽度的 15%。 如果我们不这样做，则`ax2`和`ax3`的 *y* 轴将相互重叠，并在同一轴上同时显示它们的刻度和刻度标签。
...
*   当您使用交互式后端打开该图并放大图形的某些部分时，只有 *y* 主轴域限制发生变化，而其他 *y* 轴的数据则发生变化。 右（从属轴域）与初始设置相同。 如果不使用`axes_grid1`和`axisartist`来绘制先前的图形，情况并非如此，其中所有 *y* 轴限制都会更改以反映图形的当前部分。
...
在本食谱中，我们将学习如何使用`axes_grdi1`的`AxesDivider`类在主轴域上绘制双变量图，并在主轴域的任意两侧绘制两个单变量图。 这有助于可视化两个变量之间的关系，以及同一图中两个变量的分布情况（尽管三个不同的轴/曲线）。

从技术上讲，在主轴域上绘制的变量和在主轴域两侧上的单变量图可以不同。 您可以从主轴域的四个侧面中选择任意两个侧面以进行单变量图绘制。 但是，通常的做法是绘制在主轴域的顶部和右侧。

在此配方中，我们将在带有两个变量的主轴域上绘制散点图，并且在主轴域的顶部和右侧，将为散点图所使用的两个变量分别绘制直方图。
...
4.  定义轴分割器，以使一个轴位于主轴域的顶部，另一个轴位于主轴域的右侧，如下所示：
...
5.  使刻度标签对顶部轴的 *x* 轴以及右轴的 *y* 轴不可见，以使它们不与 主轴域：
...
*   `axtop = divider.append_axes("top", size=1.5, pad="15%", sharex=axmain)`在主轴域上方定义一个新轴，如下所示：
    *   `"top"`指定要在当前轴之上绘制的新轴。
    *   `size=1.5`指定此新轴的高度为 1.5 英寸，其宽度与主访问宽度相同。
    *   `pad=15%`指定主轴域与顶部新轴之间的间隙为主轴域高度的 15%。
    *   `sharex=axmain`指定此新轴应与主轴域共享 *x* 轴属性，例如轴标签，刻度和刻度标签
*   `axright = divider.append_axes("right", "100%", 0.1, sharey=axmain)`在主轴域右侧定义了一个新轴，如下所示：
    *   `"right"`指定新轴应绘制在主轴域的右侧。
    *   `100%`指定新轴的宽度与主轴域的宽度相同。 新轴的高度也与主轴域的高度相同，因为它们都共享 *y* 轴。
    *   `0.1`是主轴域和该新轴之间的填充（以磅为单位）。
    *   `sharey=axmain`指定此新轴应与主轴域共享 *y* 轴属性，例如轴标签，刻度和刻度标签。
...
在 第 6 章和“带有高级功能的绘图” 中，我们学习了如何使用 Matplotlib 的面向对象界面在主轴域内绘制插入轴 。 在本食谱和下一篇食谱中，我们将学习如何使用`axes_grid1`工具包的`inset_locator`帮助器功能创建插入轴并绘制图像和其他图形。

在本食谱中，我们将学习如何放大图像的一部分并将其显示在相同主轴域的插入轴上。
...
*   `extent = [-2, 2, -3, 1]`指定主轴域的 x 坐标上的数据限制为`(-2, 3)`和 *y* 为`(-3, 1)`。 有关更多详细信息，请参考第 6 章，“带有高级功能的绘图”，“起源和范围”中的配方。
...
*   `axins1 = zoomed_inset_axes(ax1, 3, loc=1)`定义插入轴如下：
    *   `ax1`是要在其上创建插入轴的轴。
    *   `3`指定插入轴数据比例是主轴域的三倍，因此对于 *x* 和 *y* 轴，主轴域上的一个单位缩放到插入轴上的三个单位。
    *   `loc=1`指定将插入轴放置在主轴域的右上角。
*   `axins1.imshow(extended_image, extent=extent, interpolation="nearest", origin="lower")`在插入轴上绘制相同的扩展图像，并使用与主轴域相同的参数。
...
在本食谱中，我们将学习在主轴域上绘制散点图，并在两个插入轴上创建两个变量中每个变量的直方图。 我们将使用熟悉的`Iris`数据集来执行此操作。
...
*   `inset_ax2 = inset_axes(ax, width=1.5, height="25%", loc=2)`定义第二插入轴如下：
    ...
    *   `height="25%"`指定插入轴的高度为主轴域高度的`25%`。
    *   `loc=2`指定将插入轴放置在主轴域的左上角。
