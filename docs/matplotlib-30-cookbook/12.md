...
在第 4 章和“开发可视化来提高发布质量”中，我们学习了如何使用双轴方法在图的左右轴上绘制两个不同的比例尺。 在第 9 章和“开发交互式绘图”中，我们学习了如何在 *y* 轴上绘制不同的测量单位，并再次使用`twinx`方法进行交互式分析 。 在本食谱中，我们将学习如何使用 Matplotlib `twinx`主方法以及`axisartist`和`axes_grid1`工具包在 *y* 轴上绘制多个比例尺。
...
在 第 6 章和“带有高级功能的绘图” 中，我们学习了如何使用 Matplotlib 的面向对象界面在主轴内绘制插入轴 。 在本食谱和下一篇食谱中，我们将学习如何使用`axes_grid1`工具包的`inset_locator`帮助器功能创建插入轴并绘制图像和其他图形。
...
*   `extent = [-2, 2, -3, 1]`指定主轴的 x（-2，3）和 *y* （-3，1）坐标上的数据限制。 有关更多详细信息，请参考第 6 章，“带有高级功能的绘图”，*起源和范围*中的配方。
