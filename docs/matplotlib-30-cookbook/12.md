...
*   `ax3.spines["right"].set_position(("axes", 1.15))`将第二个 *y* 轴向右移动主轴宽度的 15%。 如果我们不这样做，则`ax2`和`ax3`的 *y* 轴将相互重叠，并在同一轴上同时显示它们的刻度和刻度标签。
...
*   `axright = divider.append_axes("right", "100%", 0.1, sharey=axmain)`在主轴右侧定义了一个新轴，如下所示：
    ...
    *   `sharey=axmain`指定此新轴应与主轴共享 *y* 轴属性，例如轴标签，刻度和刻度标签。
...
*   `axins1 = zoomed_inset_axes(ax1, 3, loc=1)`定义插入轴如下：
    *   `ax1`是要在其上创建插入轴的轴。
    *   `3`指定插入轴数据比例是主轴的三倍，因此对于 *x* 和 *y* 轴，主轴上的一个单位缩放到插入轴上的三个单位。
