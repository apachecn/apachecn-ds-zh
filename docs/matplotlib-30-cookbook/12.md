...
在本食谱中，我们将学习如何使用`axes_grdi1`的`AxesDivider`类在主轴域上绘制双变量图，并在主轴域的任意两侧绘制两个单变量图。 这有助于可视化两个变量之间的关系，以及同一图中两个变量的分布情况（尽管三个不同的轴域/曲线）。
...
*   `cax1 = ax2_divider.append_axes("right", size="7%", pad=0.03)`使用`AxesDivider`定义了颜色栏的新轴域。
*   `"right"`指定将新轴域放置在`ax2`的右侧。
*   `size="7"`指定新轴域的宽度应为`ax2`宽度的 7%。
*   `pad=0.03`指定`ax2`轴域和颜色栏轴域之间的间隙应为`ax2`宽度的 3%。
...
在 第 6 章和“带有高级功能的绘图” 中，我们学习了如何使用 Matplotlib 的面向对象界面在主轴域内绘制插入轴域。 在本食谱和下一篇食谱中，我们将学习如何使用`axes_grid1`工具包的`inset_locator`帮助器功能创建插入轴域并绘制图像和其他图形。

在本食谱中，我们将学习如何放大图像的一部分并将其显示在相同主轴域的插入轴域上。
...
在本食谱中，我们将学习在主轴域上绘制散点图，并在两个插入轴域上创建两个变量中每个变量的直方图。 我们将使用熟悉的`Iris`数据集来执行此操作。
...
*   `inset_ax1.set_title('sepal legth', size=10, pad=20)`打印尺寸为`10`点的插入轴域标题，并将其定位在顶部轴域上方`20`点，以使刻度标签和标题不重叠。
