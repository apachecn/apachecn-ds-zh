...
在第 4 章和*开发可视化以提高发布质量*中，我们学习了如何使用双轴方法在图的左右轴上绘制两个不同的比例尺。 在第 9 章和*开发交互式图*中，我们学习了如何在 *y* 轴上绘制不同的测量单位，并再次使用`twinx`方法进行交互式分析 。 在本食谱中，我们将学习如何使用 Matplotlib `twinx`主方法以及`axisartist`和`axes_grid1`工具包在 *y* 轴上绘制多个比例尺。
...
*   `ax1.legend(graphs, [g.get_label() for g in graphs], loc=5)`绘制图例如下：
    ...
    *   `loc=5`指定将图例绘制在轴的右侧。 有关放置图例的更多选项，请参考第 4 章和*开发可视化以提高发布质量*。
...
*   我们为标签，刻度和刻度标签设置各种属性的方式与我们使用 axisartist 工具包在第 12 章和*中了解的方法相同。*
...
在第 3 章，*绘制多个图形，子图形和图形*和第 6 章，*绘制高级功能*的过程中，我们学习了如何绘制 网格上具有多个网格选项的多个图。 当您尝试使用带有这些选项的网格上的颜色条来绘制图像时，颜色条对齐可能不是我们想要的方式。 `axes_grid1`工具包的`ImageGrid`辅助功能可在网格中以正确对齐方式绘制带有颜色条的图像。 我们将在本食谱中学习如何使用这些功能。
...
在 第 6 章和*具有高级功能的绘图* 中，我们学习了如何使用 Matplotlib 的面向对象界面在主轴内绘制插入轴 。 在本食谱和下一篇食谱中，我们将学习如何使用`axes_grid1`工具包的`inset_locator`帮助器功能创建插入轴并绘制图像和其他图形。
...
*   `extent = [-2, 2, -3, 1]`指定主轴的 x（-2，3）和 *y* （-3，1）坐标上的数据限制。 有关更多详细信息，请参考第 6 章，*具有高级功能的绘图*，*起源和范围*中的配方。
