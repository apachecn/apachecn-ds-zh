...
这是图形显示在输出设备上的最底层。 这可以是 Matplotlib 支持的任何用户界面。 后端有两种类型：**用户界面后端**（用于`pygtk`，`wxpython`，`tkinter`，`qt4`或`macosx`等，也称为[ **交互式后端**）和**硬拷贝**后端，以制作图像文件（`.png`，`.svg`，`.pdf`和`.ps`，也称为**非 交互式后端**）。 我们将在稍后的第 9 章，*开发交互式图*和第 10 章，*将图形嵌入图形用户界面*中学习如何配置这些后端。 ]。
