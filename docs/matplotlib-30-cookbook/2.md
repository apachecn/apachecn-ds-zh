...
线图用于表示两个连续变量之间的关系。 它通常用于表示随时间变化的趋势，例如季度和年度的 GDP 增长率，通货膨胀，利率和股票价格。 我们在第 1 章和 *Matplotlib* 剖析中看到的所有图形都是线图的示例。
...
*   `plt.subplots()`允许我们根据图形的数量以及如何在图形中组织图形来定义图形的布局。 我们将在第 3 章和*绘制多个图表，子图和图形*和第 6 章和*具有高级功能的绘图*中了解更多信息。 。 在这种情况下，我们使用它来访问要绘制条形图的轴，以便可以用代表条形图的实际数据对其进行注释。 如果您从第 1 章， *Matplotlib 解剖图*中回想起，我们已经看到轴是图形中的单个图。
...
*   `ax.legend(loc=2)`指定要在图的左上方绘制的图例； 我们将在第 4 章和*开发可视化以提高发布质量中了解有关图例位置的更多信息。*
...
*   `plt.cm.OrRd()`指定强度变化的颜色范围； Matplotlib 中有预定义的颜色图。 我们将在第 4 章和*开发可视化以提高发布质量*中介绍这些内容。 在这里，它用于以不同的颜色显示每年的数据。
...
在`plt.boxplot()`中，通过指定`showfliers=False` 自变量 ，您可以抑制离群值，因此仅在两侧的晶须上绘图。 通过指定 `vert=False` 自变量 ，您可以水平绘制箱形图，还可以定制具有不同形状和颜色的离群值：
