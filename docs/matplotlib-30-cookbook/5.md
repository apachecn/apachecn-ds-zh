...
*   `ax1 = fig.add_axes([0.0, 0.55, 0.4, 0.4])`向图形对象添加新轴域：
    ...
    *   在此轴域上绘制了一个正弦图
*   `ax2 = fig.add_axes([0.0, 0.1, 0.4, 0.4])`是第二个轴，它的大小也与和的第一个相同，从到的最左端开始，是图形高度从底部开始的 10%。 在此轴域上绘制直方图。
*   `ax3 = fig.add_axes([0.5, 0.1, 0.4, 0.85])`是第三根轴。 它从左侧图形的 50% 开始，从底部底部图形的 10% 开始，的宽度是的图形宽度的 40% ， 的高度为身高的的 85%。 在此轴域上绘制了一个绿色多项式。
...
*   `ax2 = fig.add_subplot(gs1[0, 1:])`为的第二个和的第一行的的第三个单元格创建轴实例`ax2`的。 在此轴域上绘制直方图。
*   `ax3 = fig.add_subplot(gs1[1:, 0])`为的第二个和的第一列的中的第三个单元创建轴实例`ax3`。 在该轴域上绘制了 500 个随机数的散点图。
