...
对于本章中的示例，我们将使用与第 2 章和*基本绘图入门*中介绍的相同的`Wine Quality`和`Iris`数据集。
...
Matplotlib 还提供了另一个名为`AxesGrid`的工具包，它涵盖了更高级的网格选项，包括网格中的图像。 我们将在第 13 章和*中使用 axis_grid1 工具包*进行介绍。
