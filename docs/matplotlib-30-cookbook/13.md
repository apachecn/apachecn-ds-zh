...
*   `shapename = 'admin_1_states_provinces_lakes_shp'`指定要从*自然地球*数据站点下载以绘制地图的形状文件的名称。 它不是确切的文件名，而是可用的各种类型中的文件类型。 类型很多，例如`admin 0`，`admin 1`，`populated places`，`railroads`，`roads`，`airports`和`ports`。 有关详细信息，请参考*自然地球*数据站点，该站点可在 [https://www.naturalearthdata.com/downloads/10m-cultural-vectors/](https://www.naturalearthdata.com/downloads/10m-cultural-vectors/) 上找到。
