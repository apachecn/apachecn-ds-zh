...
7.  使用`Mercator`投影定义第二个绘图的轴域。 应用背景图像，并像以前一样添加海岸线特性：
...
*   `ax1.add_feature(cfeature.STATES, facecolor='wheat')`绘制了美国各州的内部边界。 Cartopy 将此作为一种特性，类似于我们先前了解的其他物理特性，例如陆地，海洋，河流和湖泊。 对于其他国家/地区，我们将不得不从形状文件中获取数据并将其单独绘制。
...
3.  使用`ShapelyFeature`函数读取时区形状文件的内容作为特性：
