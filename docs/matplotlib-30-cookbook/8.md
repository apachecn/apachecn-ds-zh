...
*   `axmain.set(xlim=(-5, 5), ylim=(-75, 175), autoscale_on=False, title='Click to zoom')`设置属性，例如 *x* 和 *y* 轴限制，自动缩放是打开还是关闭，以及是的标题，还是。 主轴域，`axzoom.set(xlim=(-2, 2), ylim=(-8, 8), autoscale_on=False, title='Zoom window')`为缩放轴定义了和相同的属性。
...
在本食谱中，我们将学习如何捕获图形输入和离开事件以及轴输入和离开事件，并使用它们来更改图形和轴域的  属性，并使轴上的线图可见 /无形。
...
在第 4 章，“开发可视化来提高发布质量”的“双生轴域”配方中，我们学习了如何在左侧和上绘制两个不同的比例尺 两个独立变量的右轴。 当我们在交互模式下使用此功能时，的缩放和平移功能也可以正常工作，因为两个变量是独立的。 但是，当两个变量相互依赖时，例如以弧度和度为度量单位； 公斤和磅； 或华氏温度，摄氏温度，开尔文温度和朗肯温度，那么两个变量必须同步。

在本食谱中，我们将通过一个温度示例学习如何做到这一点。 我们将在，主轴域上使用，摄氏温度单位，并在从属轴域上使用华氏，开尔文和朗肯。
...
*   `def prev(self, event):`将`prev`方法定义为`prev`按钮的回调函数：
    ...
    *   `axs.clear()`清除轴上的先前数据。 如果不这样做，则所有群集都将在轴上可见，并且仅刷新的群集的颜色将根据属性周期中设置的默认颜色进行更改。
...
*   `axprev = plt.axes([0.7, 0.005, 0.1, 0.05])`定义图中，`prev`按钮的轴，`axnext = plt.axes([0.81, 0.005, 0.1, 0.05])`定义图中`next`按钮的轴。 如第 6 章中的解释，“带有高级功能的绘图”，“控制轴位置”配方，轴的定义是从左，下，宽度和高度， 它们位于轴域坐标系中。
...
*   `ax = fig.add_axes([0, 0, 1, 1], frameon=False)`设置轴，`frameon=False`指定不绘制脊线。
