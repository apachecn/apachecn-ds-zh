...
在本食谱中，我们将通过一个温度示例学习如何做到这一点。 我们将在，主轴上使用，摄氏温度单位，并在寄生轴上使用华氏，开尔文和朗肯。
...
2.  根据，的电流限制，  主轴上的摄氏刻度来定义更新所有刻度的功能：
...
*   `def refresh_scales(ax_c):`是根据当前摄氏温度限制刷新和寄生轴上所有刻度的功能：
...
*   `ax_r = ax_c.twinx()`定义了主轴上朗肯刻度的寄生轴。
...
*   `ax_f.yaxis.label.set_color('b')`设置华氏度的 *y* 轴标签的颜色； 通常的`set_ylabel(color='b')`不适用于寄生轴。
