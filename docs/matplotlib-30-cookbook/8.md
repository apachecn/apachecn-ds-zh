...
*   `figure_exit(event)`是响应`figure_leave_event`的回调函数。 此功能还将文本打印到`sysout`，将图形的前景颜色设置为*红色*，并绘制更新的图形。
...
在第 4 章，“开发可视化来提高发布质量”的“双生轴域”秘籍中，我们学习了如何在左和右轴域绘制两个不同的比例尺的两个独立变量。 当我们在交互模式下使用此功能时，的缩放和平移功能也可以正常工作，因为两个变量是独立的。 但是，当两个变量相互依赖时，例如以弧度和度为度量单位； 公斤和磅； 或华氏温度，摄氏温度，开尔文温度和朗肯温度，那么两个变量必须同步。
