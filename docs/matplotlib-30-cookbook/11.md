...
`axisartist`使用自 Matplotlib 主`Axes`类派生的自定义`Axes`类，其中每个轴（左，下，上和右）由单独的艺术家管理。 由于`ax.axis`的作用类似于字典，因此`ax.axis["left"]`，`ax.axis["bottom"]`，`ax.axis["top"]`和`ax.axis["right"]`可以访问它们。 我们还可以根据需要定义其他轴。
