...
逼近问题的目标是找到相对于参数`a`最小化表达式`||f(x) - g[a](x)||`的逼近成员。 这等效于相对于`a`为`error`功能`F(a) = ||f(x) - g[a](x)||`找到`a`（局部或全局）最小值。
...
尽管从技术上讲，`leastsq`算法主要处理单变量函数的逼近，但可以借助索引，散布，解包（使用特殊的`*`运算符）和稳定的和来处理多元函数。
