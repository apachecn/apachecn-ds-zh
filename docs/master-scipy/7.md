...
在任何给定情况下，我们都可以使用在最小二乘意义上的近似和插值探索中学到的工具，这些知识在[第 1 章](../Text/1.xhtml#aid-DB7S2 "Chapter 1\. Numerical Linear Algebra")，*数值线性代数*和[中 第 2 章](../Text/2.xhtml#aid-K0RQ2 "Chapter 2\. Interpolation and Approximation")和*插值和近似*。 两个库`scipy.stack`和`statsmodels`中以及工具包`scikit-learn`中都有许多工具可以执行此操作和相关分析：
