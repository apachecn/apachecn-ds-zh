...
我们通过`scikit-learn`工具包的子模块`sklearn.cluster`中的类`MeanShift`在 SciPy 堆栈中实现。 该算法的要素之一是使用径向基函数（在[第 1 章](../Text/1.xhtml#aid-DB7S2 "Chapter 1\. Numerical Linear Algebra")，*数值线性代数*中讨论）进行近似，为此我们需要提供适当的带宽。 该算法（如果未提供）将尝试根据数据进行估算。 此过程可能非常缓慢且昂贵，并且通常由我们自己进行估算是一个好主意，因此我们可以控制资源。 我们可以使用同一子模块中的辅助函数`estimate_bandwidth`来实现。
