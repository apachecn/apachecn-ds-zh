...
我们前面提到的所有工具（`setuptools`，`Distutils`和`numpy.distutils`）都围绕功能设置。 为了了解大多数包装要求，我们将研究一个简单的设置功能，然后研究一个成熟的安装程序。 要创建基本的安装程序，我们需要使用有关包的元数据调用安装程序功能。 让我们叫第一个包`py_hello`，它只有一个功能`greeter`，并且在调用时只打印一条消息。 可从 [Bitbucket 存储库下载该包](https://bitbucket.org/tdatta/books/src/af376df081ef/python/simple_setup/?at=master)。该项目的目录结构如下：
